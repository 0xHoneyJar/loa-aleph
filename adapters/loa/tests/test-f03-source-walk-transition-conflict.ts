...
import {deriveSourceWalkCompletion} from '../../../scripts/lib/source-walk-transition.ts';
// Originally reproduced C-02 at ad8be4e. Its original output and source remain
// in that immutable checkpoint and the historical stop evidence. This successor
// tests the clarified 1.9 rule while retaining the predecessor discriminator.
// Synthetic Core records only: no CLI/native/model execution.
...
try {
 for(const version of ['1.8.0-provisional','1.9.0-provisional']) {
  ...
  let afterWalk=readFileSync(join(after,walkPath),'utf8');const afterPackets=readFileSync(join(after,packetPath),'utf8');
  let complete=afterWalk.split('\n').find(line=>line.startsWith('| SRC-701 |'))!;
  ...
  if(version==='1.9.0-provisional'){
   const projection=deriveSourceWalkCompletion(loadRun(before),loadRun(after));
   afterWalk=Buffer.from(projection.after_base64,'base64').toString('utf8');
   complete=projection.completions[0].after_row;writeFileSync(join(after,walkPath),afterWalk);
   assert.equal(projection.completions[0].before_row,blocked);
  }
  ...
  const replaced=plan(after);assert.equal(replaced.result,version==='1.9.0-provisional'?'PASS':'FAIL');
  if(version==='1.8.0-provisional')assert.match(replaced.error!,/subject writes may insert rows but cannot replace retained lines/u);
  ...
  assert.equal(plan(stale).result,version==='1.9.0-provisional'?'FAIL':'PASS');assert(k214(stale).some(c=>c.status==='FAIL'&&c.message.includes('last recorded cursor')));
  ...
  assert.equal(plan(append).result,version==='1.9.0-provisional'?'FAIL':'PASS');assert(k214(append).some(c=>c.message.includes('more than one final traversal state')));
