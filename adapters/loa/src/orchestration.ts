...
const clockDefault: Clock = { now: () => new Date().toISOString() };
const heldLocks = new Set<string>();
type Sealed<T> = T & { digest: string };
interface BasisMember {
  path: string; digest: string; byte_length: string; storage: 'blob' | 'immutable-reference';
}
interface WorkBasis {
...
interface WorkIdentity {
  run_id: string;
  pins: LoaRunState['identity'];
  checkpoint: string;
  ledger: LoaRunState['ledger'];
  ordinal: string;
  work: Extract<NextWork, { kind: 'local' | 'worker' }>;
  dependencies: Array<{ work_id: string; consumption_digest: string; call_id: string; receipt_digest: string }>;
}
export interface OrchestrationWork {
  format: 'aleph-loa-work-item/v1';
  work_id: string;
  identity: WorkIdentity;
  created_at: string;
  basis_digest: string;
  core_stage_contract: { path: string; digest: string };
  call: null | {
    call_id: string;
    role: LoaRoleId;
    ...
    allowlist: Array<{ path: string; digest: string }>;
    withheld: Array<{ selector: string; core_ref: string }>;
    producer_context_id: string | null;
    downstream_operations: [];
    model: LoaRunState['identity']['models'][LoaRoleId];
  };
}
interface Acceptance {
  format: 'aleph-loa-accepted-return/v1';
  work_id: string; work_digest: string; call_id: string; checkpoint: string;
  basis_digest: string;
  request_digest: string;
  native: Array<{ path: string; digest: string; byte_length: string }>;
  raw_digest: string; validation_digest: string; validated_digest: string; contract_digest: string;
  context_id: string; producer_context_id: string | null; simulation: boolean;
}
export interface OrchestrationCommit {
  format: 'aleph-loa-orchestration-commit/v1';
  work_id: string; work_digest: string; acceptance_digest: string | null;
  plan_digest: string; before_checkpoint: string; before_chain: string;
  journal: string;
}
interface Consumption {
  format: 'aleph-loa-work-consumption/v1';
  work_id: string; commit_digest: string; journal_digest: string;
  after_checkpoint: string; after_chain: string;
}
interface DispatchIntent {
  format: 'aleph-loa-dispatch-intent/v1';
  work_id: string; work_digest: string; call_id: string;
  invocation_digest: string; request_digest: string; checkpoint: string;
  pid: string; created_at: string;
}
export interface AuthenticatedWork {
  work: Sealed<OrchestrationWork>;
  acceptance: Sealed<Acceptance> | null;
  returned: ValidatedWorkerReturn | null;
  transition: WorkTransition;
  beforeState: LoaRunState;
  chainBefore: string;
}
function canonicalPath(runDir: string, path: string): string {
  assertSafeRelativePath(path);
  const full = join(runDir, path);
  assertNoSymlinkComponents(runDir, full);
  return full;
}
function immutable(runDir: string, path: string, bytes: Buffer): void {
  ...
  try {
    ...
    try { linkSync(staged, full); } catch (error) {
...
function seal<T extends object>(value: T): Sealed<T> {
  return { ...value, digest: sha256Digest(stableJsonBytes(value)) };
}
function publish<T extends object>(runDir: string, path: string, value: T): Sealed<T> {
  const sealed = seal(value);
  immutable(runDir, path, stableJsonBytes(sealed));
  return sealed;
}
function readSealed<T extends object>(runDir: string, path: string, keys: readonly string[]): Sealed<T> {
  const full = canonicalPath(runDir, path), bytes = readStableRegularFile(full).bytes;
  const value = parseStrictJson(bytes) as Record<string, unknown>;
...
function acceptedPath(callId: string): string {
  assertWork(CALL_ID.test(callId), 'WORK_CALL_IDENTITY', callId);
  return `${ROOT}/accepted/${callId}.json`;
}
export function orchestrationCommitPath(id: string): string { workPath(id); return `${ROOT}/commits/${id}-intent.json`; }
function consumedPath(id: string): string { workPath(id); return `${ROOT}/commits/${id}-consumed.json`; }
function execution(state: LoaRunState): WorkExecution {
...
function captureBasis(runDir: string): Sealed<WorkBasis> {
  ...
  for (const full of walkRegularFiles(runDir)) {
    ...
    if (storage === 'blob') immutable(runDir, `${ROOT}/blobs/${digest.slice(7)}`, bytes);
...
function basisBytes(runDir: string, member: BasisMember): Buffer {
...
function withBasis<T>(runDir: string, digest: string, action: (root: string) => T): T {
...
export function assertRecoveryPrerequisites(runDir: string, work: Sealed<OrchestrationWork>, plan: WorkTransition): void {
  ...
  for (const member of basis.members.filter((entry) => !entry.path.startsWith('control/'))) {
    ...
    assertWork(bytes.equals(basisBytes(runDir, member)) || effect && bytes.toString('base64') === effect.after_base64,
      'WORK_PREREQUISITE_CHANGED', member.path);
  }
}
export function readOrchestrationWork(runDir: string, id: string): Sealed<OrchestrationWork> {
  ...
  assertWork(work.core_stage_contract.path === WORK_STAGE_CONTRACT
    && work.core_stage_contract.digest === verifyAndLoadLoaBundle(runtime.bundle.root).files.get(WORK_STAGE_CONTRACT)?.digest,
  'WORK_CORE_CONTRACT', id);
...
function allWorkIds(runDir: string): string[] {
...
export function reopenAcceptedWorkReturn(runDir: string, workId: string): {
  work: Sealed<OrchestrationWork>; receipt: Sealed<Acceptance>; returned: ValidatedWorkerReturn;
} {
  ...
  for (const [name, bytes] of [['raw.json', checked.rawBytes], ['validation.json', stableJsonBytes(checked.report)],
    ['validated.json', returned.canonicalBytes()]] as const) {
...
function retainedNativeDigests(runDir: string, callId: string, simulated: boolean): Acceptance['native'] {
...
export function acceptOrchestrationReturn(runDir: string, callId: string) {
  return withOrchestrationLock(runDir, () => {
    ...
    const receipt = publish<Acceptance>(runDir, acceptedPath(callId), {
...
function assertDispatchIntent(runDir: string, work: Sealed<OrchestrationWork>, request: WorkerRequest, invocationDigest: string): void {
  const intent = readSealed<DispatchIntent>(runDir, `${ROOT}/dispatch/${request.call_id}-intent.json`,
    ['format', 'work_id', 'work_digest', 'call_id', 'invocation_digest', 'request_digest', 'checkpoint', 'pid', 'created_at']);
...
export function beginOrchestrationDispatch(runDir: string, request: WorkerRequest, invocationDigest: string): 'spawn' | 'retained' {
  return withOrchestrationLock(runDir, () => {
    ...
    publish<DispatchIntent>(runDir, path, { format: 'aleph-loa-dispatch-intent/v1', work_id: work.work_id, work_digest: work.digest,
...
export function completeOrchestrationDispatch(runDir: string, request: WorkerRequest, invocationDigest: string, publishResult: () => void): void {
...
function valueOf(receipt: Sealed<Acceptance>, returned: ValidatedWorkerReturn, role: string): WorkValue {
...
export function prepareOrchestrationCommit(runDir: string, authenticated: AuthenticatedWork): Sealed<OrchestrationCommit> {
  const work = authenticated.work;
  return publish<OrchestrationCommit>(runDir, orchestrationCommitPath(work.work_id), {
...
export function readOrchestrationCommit(runDir: string, id: string): Sealed<OrchestrationCommit> {
...
export function recordOrchestrationConsumption(runDir: string, id: string, state: LoaRunState): void {
  const intent = readOrchestrationCommit(runDir, id);
  publish<Consumption>(runDir, consumedPath(id), { format: 'aleph-loa-work-consumption/v1', work_id: id, commit_digest: intent.digest,
...
function transportAction(runDir: string, work: Sealed<OrchestrationWork>): Record<string, JsonValue> {
  ...
  if (!existsSync(bundleRoot)) assembleWorkerBundle({ bundle: verifyAndLoadLoaBundle(runtime.bundle.root), runDir, callId: call.call_id, runId: work.identity.run_id,
    stage: work.identity.work.obligation.stage as CoreStage, role: call.role, kind: call.kind,
...
export function proposeOrchestrationSamples(runDir: string, raw: Buffer, clock: Clock = clockDefault): void {
  withOrchestrationLock(runDir, () => {
    ...
    assertWork(selected.kind === 'proposal' && selected.operation === 'criteria.samples',
      'WORK_PROPOSAL_WINDOW', 'criteria samples are not the first unmet preparation');
