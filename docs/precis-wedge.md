...
9. No projection is generated *by the Précis itself*; the Précis stays
   ...
   see `docs/decisions/0001-projection-as-separate-downstream-stage.md` — which is
   built in its own later slices, not in this wedge.)
...
produce PRDs, doctrines, specs, etc.) is real and Aleph-owned, but it is built in
its own later slices — see
...
- No PRD, product-doctrine, or spec produced *in this wedge* (deferred to the
  projection-stage slices).
- No endpoint.
