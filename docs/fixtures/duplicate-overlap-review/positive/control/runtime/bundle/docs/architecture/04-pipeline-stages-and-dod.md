...
### S4 successor semantic preservation (1.7)

After an existing judgment proposes an exact new lineage successor, the
normalizer may express only that successor with complete semantic facets and
direct predecessor provenance. Fresh L2S must uphold preservation before
admission. This does not choose duplicate groups or change L3 membership or
judgment. Failure blocks the successor. At C1, after current semantic and
canonical relation obligations close, seal the semantic ledger alongside
representation-use closure with semantic_review_closure_hash in the same
event. C2 and C3 consume closure; they cannot rewrite it.
...
## Semantic prerequisites in run format 1.7

Activation is cumulative semantic-unit-review. Every S2 candidate selector
and S3 claim/no-claim/material selector has a sealed SEM and fresh L2S; S3
coverage is exhaustive only for 1.7. L1 gap review and L2F remain independent.
S2 raw capture preserves every exact fragment, one PKT per fragment, walk
position/order and gap accounting. Semantic challenges retain captured bytes.
S3 affirmative admission requires exact reviewed sole proposition and packet
provenance, complete source-supported facets, no blocking unknowns, usable
material and required L2F. No-claim needs its reviewed proposal and LIN event.
Every upheld S2 unit has reviewed S3 claim/no-claim accounting.

The exact T3.7 stage-seal and C1 grammar governs. Pending semantic work blocks
exit; unresolved-record completion cannot waive source-walk or lineage DoDs.
Précis section 17 mechanically retains every historical producer/reviewer
finding, including a predecessor followed by a narrower usable claim.
Manual mode remains the only sanctioned execution path.

## S4 duplicate and overlap review (1.8)
