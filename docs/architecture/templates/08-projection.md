...
the hash recorded here must verify unchanged at P3.
...
- `statement kind`: `load-bearing` (asserts something a reader could act on)
  | `open-item` (must trace to a deferred/unresolved claim and be phrased as
  open) | `gap` (template wanted it; Précis lacks it; phrased as explicit
  gap; `backing` empty by definition) | `scaffolding` (headings, transitions;
  asserts nothing; `backing` empty).
- Rules (K6 enforces): every `load-bearing` row has ≥1 CC id, each resolving
  to carried/merged; an `open-item` asserting language is a defect; no
  statement may rest on claims governed by a do-not-use boundary; every
  rendered document paragraph appears in some row (coverage).
