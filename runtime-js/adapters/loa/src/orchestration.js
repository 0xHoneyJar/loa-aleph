import { existsSync, readFileSync, mkdirSync, mkdtempSync, rmSync, linkSync, unlinkSync, openSync, closeSync, fsyncSync, readdirSync, lstatSync, writeFileSync, } from 'node:fs';
...
import { selectNextWork, deriveWorkTransition, workDigest, workJson, assertWork, WORK_STAGE_CONTRACT, WORK_TRANSITION_CAPABILITY, CRITERIA_SAMPLE_INPUT_PATH, criteriaSampleProposal, } from '../../../scripts/lib/work-transitions.js';
...
export function readOrchestrationWork(runDir, id) {
    const work = readSealed(runDir, workPath(id), ['format', 'work_id', 'identity', 'created_at', 'basis_digest', 'core_stage_contract', 'call']);
...
export function deriveAuthenticatedWork(runDir, id, recovering = false) {
    ...
    const derived = withBasis(runDir, work.basis_digest, (basis) => {
        ...
        return { beforeState, chainBefore: existsSync(chainPath) ? readFileSync(chainPath, 'utf8') : '',
            transition: deriveWorkTransition(loadRun(basis), execution(beforeState), work.identity.work, accepted ? valueOf(accepted.receipt, accepted.returned, work.call.role) : null, work.created_at) };
    });
...
function createWork(runDir, selected, clock) {
    ...
    if (selected.kind === 'worker') {
        ...
        const coreRef = coreBlindPolicyReference(verifyAndLoadLoaBundle(runtime.bundle.root), role, selected.obligation.stage, selected.call.task_line);
