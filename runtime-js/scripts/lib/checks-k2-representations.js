...
import { SUPPORTED_RUN_FORMAT_VERSIONS, usesFormalLayoutBindings } from './run-model.js';
import { REPRESENTATION_ASSET_PATH, REPRESENTATION_PATH, REPRESENTATION_USE_PATH, validateRepresentationRun } from './source-representation.js';
export function runK2Representations(results, model) {
    ...
    if (!usesFormalLayoutBindings(version) && !present
        && (!version || SUPPORTED_RUN_FORMAT_VERSIONS.includes(version)))
        return;
    results.run('K2.18', 'source representation bindings (STRUCTURAL ONLY)', (fail) => {
        ...
        if (!present && model.manifest?.states.every((row) => ['DRAFT', 'BLOCKED'].includes(row.values.state)))
            return 'DRAFT capture not prepared';
