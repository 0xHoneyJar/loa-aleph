...
import { hasRunCapability, SUPPORTED_RUN_FORMAT_VERSIONS } from './run-model.js';
import { SEMANTIC_PATH, validateSemanticRun } from './semantic-review.js';
export function semanticArtifactsPresent(model) {
...
export function runK2Semantics(results, model) {
    const version = model.manifest?.runFormatVersion || '', active = hasRunCapability(version, 'semantic-unit-review');
    if (!active && !semanticArtifactsPresent(model) && (!version || SUPPORTED_RUN_FORMAT_VERSIONS.includes(version)))
        return;
    results.run('K2.19', 'semantic subject and review accounting (STRUCTURAL ONLY)', () => {
        if (!active)
            throw new Error(`SEM_COMPATIBILITY ${version}: canonical semantic artifacts require semantic-unit-review`);
