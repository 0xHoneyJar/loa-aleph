...
function ambiguitySignals(model) {
    ...
    for (const document of model.documents.values()) {
        if (!document)
            continue;
        if (document.relativePath === 'ledgers/internal-ambiguities.md')
            signals.push(document.relativePath);
        if (/internal_ambiguity_format\s*:/u.test(document.text))
            signals.push('format marker');
        if (document.tables.some((table) => tableLooksLike(table, 'ambiguity_id')))
            signals.push('T5 table');
    }
...
function locatorSpan(path, locator) {
...
function verdictForTarget(model, target) {
    const matches = [];
    for (const document of model.documents.values()) {
        if (!document?.relativePath.startsWith('verification/harness/'))
            continue;
...
function sameSourceEntity(model, row) {
    if (row.values.sourceEntityKind === 'PKT') {
        return model.packets.some((packet) => (packet.values.packetId === row.values.sourceEntityId
            && packet.values.sourceId === row.values.sourceId));
    }
    if (row.values.sourceEntityKind === 'CC') {
        return model.claims.some((claim) => (claim.values.claimId === row.values.sourceEntityId
...
function localScopeRefs(model, row) {
    try {
        const value = JSON.parse(row.values.searchCompletionRef);
        ...
        if (value.some((id) => !order.has(id)))
            return null;
        if (value.some((id, index) => index > 0 && order.get(value[index - 1]) >= order.get(id)))
            return null;
        return value;
    }
    catch {
...
function completionRef(model, sourceId) {
    const completion = model.sourceWalk.completions.find((row) => (row.values.sourceId === sourceId && row.values.completionState === 'complete'));
    if (!completion)
        return null;
...
function reviewSubject(definition, assessment) {
    ...
    if (!packets.clean || !candidates.clean || !relations.clean
        ...
        return null;
    return {
        source_entity_kind: definition.values.sourceEntityKind,
...
function checkControlState(model, fail) {
        ...
        try {
            ...
            if (name.endsWith('-request.json')) {
                ...
                if (!canonical.equals(bytes))
                    fail(`${name} is not exact canonical request bytes`);
                if (name !== `${request.request_id}-request.json`)
                    fail(`${name} request identity/path mismatch`);
                if (requests.has(request.request_id))
    ...
    for (const [id, request] of requests) {
        ...
        if (!prior || sequence > prior.sequence)
            latestByAssessment.set(key, { id, sequence });
    }
    ...
    for (const id of responses.keys())
        if (!requests.has(id))
            fail(`${id} has a fabricated or orphan response`);
    if (active > 1)
...
export function runK2Ambiguities(results, model) {
    results.run('K2.17', 'internal ambiguity lifecycle structure (STRUCTURAL ONLY)', (fail) => {
        ...
        if (retainedPhaseLines.length !== phases.length)
            fail('closure_phase contains an unknown value');
        ...
        if ((hasC3 || hasS5) && !hasC2)
            fail('C3 and S5 require retained C2');
        if (hasS5 && !hasC3)
            fail('S5 requires retained C3');
        ...
        const markerCount = ambiguity.document.lines.filter((line) => (/^\s*-\s*internal[_ -]ambiguity[_ -]format\s*:/iu.test(line))).length;
        ...
        for (const row of ambiguity.t5_1Rows) {
            ...
            if (!/^AMB-\d{4,}$/u.test(values.ambiguityId))
                fail(`${values.ambiguityId || 'T5.1 row'} has invalid ambiguity_id`);
            if (definitions.has(values.ambiguityId))
                fail(`${values.ambiguityId} is defined more than once`);
            else
                definitions.set(values.ambiguityId, row);
            if (!sameSourceEntity(model, row))
                fail(`${values.ambiguityId} source entity is absent or not from ${values.sourceId}`);
            if (values.sourceEntityKind === 'PKT' && !currentPackets.has(values.sourceEntityId))
                fail(`${values.ambiguityId} source packet is historical`);
            if (values.sourceEntityKind === 'CC' && !currentClaims.has(values.sourceEntityId))
                fail(`${values.ambiguityId} source claim is historical`);
            ...
            if (!exactUtf8(expression))
                fail(`${values.ambiguityId} interval is not on valid UTF-8 boundaries`);
            const decoded = canonicalBase64(values.expressionBytesBase64);
            if (!decoded || !decoded.equals(expression))
                fail(`${values.ambiguityId} expression base64 does not reopen exact bytes`);
            if (normalizeSha256(values.expressionSha256) !== sha256(expression))
                fail(`${values.ambiguityId} expression hash is wrong`);
            const locus = locatorSpan(source.path, values.expressionLocator);
            if (!locus || start < locus.start || end > locus.end)
                fail(`${values.ambiguityId} expression locator does not contain the interval`);
            const expressionKey = `${values.sourceId}\0${start}\0${end}\0${values.expressionSha256}`;
            if (expressionKeys.has(expressionKey))
                fail(`${values.ambiguityId} duplicates an exact expression definition`);
            expressionKeys.add(expressionKey);
            const packets = parseOrderedIds(values.basisPacketIds, 'PKT');
            if (!packets.clean)
                fail(`${values.ambiguityId} basis_packet_ids are not canonical`);
            let covered = false;
            for (const id of packets.ids) {
                ...
                if (packetSpan && start >= packetSpan.start && end <= packetSpan.end)
                    covered = true;
            }
            if (!covered)
                fail(`${values.ambiguityId} basis packets do not cover the expression interval`);
        ...
        for (const row of ambiguity.t5_2Rows) {
            ...
            if (!definition) {
                fail(`${values.ambiguityId || 'T5.2 row'} has no T5.1 definition`);
                continue;
            }
            const bucket = assessments.get(values.ambiguityId) || [];
            bucket.push(row);
            assessments.set(values.ambiguityId, bucket);
            const seq = exactPositive(values.assessmentSeq);
            const expectedPredecessor = seq === 1 ? 'none' : String((seq || 0) - 1);
            if (!seq || values.predecessorAssessmentSeq !== expectedPredecessor)
                fail(`${values.ambiguityId} assessment predecessor is invalid`);
            if (!SEARCH_SCOPE_KINDS.includes(values.searchScopeKind))
                fail(`${values.ambiguityId} search_scope_kind is invalid`);
            if (!CANDIDATE_STATES.includes(values.candidateState))
                fail(`${values.ambiguityId} candidate_state is invalid`);
            if (!RESOLUTION_STATES.includes(values.resolutionState))
                fail(`${values.ambiguityId} resolution_state is invalid`);
            if (!CARRY_STATES.includes(values.carryState))
            ...
            if (!affected.clean)
                fail(`${values.ambiguityId} affected_relation_ids are not canonical`);
            affected.ids.forEach((id) => { if (!relationIds.has(id))
                fail(`${values.ambiguityId} affected relation ${id} is absent`); });
            if (!legalResolutionCarryState(values.resolutionState, values.carryState, affected.ids.length))
            ...
            if (sourceRecord && packets.clean && candidates.clean && affected.clean
                ...
                const basis = {
                    ...
                    scope_kind: values.searchScopeKind,
                    ...
                    expression_sha256: definition.values.expressionSha256,
                    basis_packet_ids: packets.ids,
                    candidate_state: values.candidateState,
                    candidate_refs: candidates.candidates,
                };
            ...
            if (reviewDigests.has(values.reviewSubjectDigest))
                fail(`${values.ambiguityId} duplicates a review-subject digest`);
            reviewDigests.add(values.reviewSubjectDigest);
            if (!/^VER-\d{4,}$/u.test(values.reviewedBy))
                fail(`${values.ambiguityId} reviewed_by is invalid`);
            ...
            if (!/^(?:human|invocation):\S+$/u.test(values.proposedBy))
                fail(`${values.ambiguityId} proposed_by is invalid`);
        }
        ...
        if (hasC2) {
            for (const id of definitions.keys())
                if (!assessments.has(id))
                    fail(`${id} has no reviewed T5.2 assessment at C2`);
        }
        const materialHistories = new Map();
        for (const file of model.files.filter((entry) => (/^verification\/harness\/S4\/material-impact-subjects\/AMB-\d{4,}-A\d+-M\d+\.json$/u
            .test(entry.relativePath)))) {
            try {
                const subject = JSON.parse(file.text);
                ...
                const assessment = assessments.get(subject.ambiguity_id)?.find((row) => (Number(row.values.assessmentSeq) === subject.assessment_seq));
        ...
        const latestMaterial = new Map();
        for (const [key, history] of materialHistories) {
            history.sort((left, right) => (left.subject.material_impact_seq - right.subject.material_impact_seq));
        ...
        for (const request of control.requests.values()) {
            ...
            const material = materialHistories.get(key)?.find((entry) => (entry.subject.material_impact_seq === value.authority_subject.material_impact_seq));
        ...
        const authoritySeqs = new Map();
        const usedResponses = new Set();
        const actionsByAssessment = new Map();
        for (const row of ambiguity.t5_3Rows) {
            ...
            if (!seq)
                fail(`${values.ambiguityId} authority_seq is invalid`);
            else {
                const bucket = authoritySeqs.get(values.ambiguityId) || [];
                bucket.push(seq);
                authoritySeqs.set(values.ambiguityId, bucket);
            }
            ...
            if (!PROCEDURAL_ACTIONS.includes(values.action))
                fail(`${values.ambiguityId} T5.3 action is invalid`);
            if (values.selectedCandidateRef !== 'none')
                fail(`${values.ambiguityId} selected_candidate_ref must equal none`);
            if (!/^sha256:[a-f0-9]{64}$/u.test(values.authoritySubjectDigest))
                fail(`${values.ambiguityId} authority subject digest is invalid`);
            ...
            const responseId = values.authorityRef.match(/^authority-response:(RESP-S4-AMB-\d{4,}-A[1-9]\d*-Q[1-9]\d*)@/u)?.[1];
            ...
            if (!request || !response) {
                ...
                try {
                    ...
                    const consequence = request.value.authority_subject.action_consequences.find((entry) => entry.action === response.value.selected_action);
                    if (!consequence) {
        ...
        for (const [id, sequences] of authoritySeqs) {
            ...
            if (sequences.some((value, index) => value !== index + 1))
                fail(`${id} authority history is forked or noncontiguous`);
        }
        for (const [key, history] of actionsByAssessment) {
            ...
            const terminalIndexes = history
                .map((entry, index) => (entry.terminality === 'nonterminal'
