import { firstRunLogEntry, location, makeIndexes, reachedState, } from './check-helpers.js';
import { LINEAGE_FORMAT, LINEAGE_TYPES, lineageCurrentClaimIds, lineageCurrentPacketIds, parseLineage, parseUnitIds, } from './lineage.js';
import { idsIn } from './markdown.js';
import { SUPPORTED_RUN_FORMAT_VERSIONS, usesLineage, } from './run-model.js';
...
export function runK2Lineage(results, model) {
    results.run('K2.15', 'lineage and lineage-current closure', (fail) => {
        ...
        if (!usesLineage(version)) {
            if (structurePresent) {
                fail(`run format ${version || '(pre-versioned)'} must not be reinterpreted as ${LINEAGE_FORMAT}`);
            }
        ...
        const markerCount = lineage.document.lines.filter((line) => /^\s*-\s*lineage[_ -]format\s*:/i.test(line)).length;
        ...
        const terminalizedBy = new Map();
        const parsedRows = [];
        for (const row of lineage.rows) {
            const { lineageId, ownerStage, type, predecessors: predecessorText, successors: successorText, basis, establishedBy, } = row.values;
            ...
            if (!LINEAGE_TYPES.includes(type)) {
            ...
            if ((type === 'merge' || type === 'duplicate') && !(predecessorCount >= 2
                && successorCount === 1
                && family === 'CC'
                && successors.family === 'CC')) {
                fail(`${lineageId || 'lineage row'} ${type || 'merge/duplicate'} requires 2+ CC -> 1 new CC`);
            }
            if ((type === 'replace' || type === 'supersede') && !(predecessorCount === 1 && successorCount === 1 && sameFamily && family !== null)) {
                fail(`${lineageId || 'lineage row'} ${type || 'replacement'} requires 1 -> 1 within one unit family`);
            }
            if ((type === 'reject' || type === 'exclude') && !(predecessorCount === 1 && successorCount === 0 && family !== null)) {
                fail(`${lineageId || 'lineage row'} ${type || 'terminal event'} requires 1 -> 0`);
            }
            if (type === 'no-claim' && !(predecessorCount === 1 && successorCount === 0 && family === 'PKT')) {
                fail(`${lineageId || 'lineage row'} no-claim requires exactly one PKT predecessor and no successor`);
            }
            const newlyTerminalized = [];
            for (const id of predecessors.ids) {
                const home = id.startsWith('PKT-') ? packets : claims;
                if (!home.has(id))
                    fail(`${lineageId || 'lineage row'} predecessor ${id} does not resolve`);
                const prior = terminalizedBy.get(id);
                if (prior)
                    fail(`${id} is terminalized by both ${prior} and ${lineageId || 'another lineage row'}`);
                else
                    newlyTerminalized.push(id);
            }
            for (const id of successors.ids) {
                const home = id.startsWith('PKT-') ? packets : claims;
                if (!home.has(id))
                    fail(`${lineageId || 'lineage row'} successor ${id} does not resolve`);
                const terminalizingEvent = terminalizedBy.get(id);
                if (terminalizingEvent) {
                    fail(`${lineageId || 'lineage row'} resurrects ${id} after it was terminalized by ${terminalizingEvent}; re-establishment requires a new identifier`);
                }
            }
        ...
        for (const row of parsedRows) {
            if ((row.type === 'merge' || row.type === 'duplicate') && row.successors.length === 1) {
                ...
                const predecessorUnion = new Set();
                for (const predecessorId of row.predecessors) {
                    for (const packetId of claimPackets(claims.get(predecessorId)))
                        predecessorUnion.add(packetId);
                }
            ...
            if (row.type === 'split' && row.family === 'CC' && row.predecessors.length === 1) {
                ...
                for (const successorId of row.successors) {
                    ...
                    for (const packetId of successorPackets)
                        aggregate.add(packetId);
                }
        ...
        const s3Closed = Boolean(model.runLog?.lines.some((line) => /^##\s+.+\s+[—-]\s+S3\s+[—-]\s+exit\b/iu.test(line))
            ...
            || reachedState(model, 'PROJECTION-ACCEPTED'));
        if (s3Closed) {
            const claimedPackets = new Set();
            for (const claim of model.claims) {
                for (const packetId of idsIn(claim.values.packets, 'PKT'))
                    claimedPackets.add(packetId);
            }
        ...
        for (const merge of activeMerges) {
            ...
            const matches = parsedRows.some((row) => (['merge', 'duplicate'].includes(row.type)
        ...
        for (const row of parsedRows.filter((candidate) => ['merge', 'duplicate'].includes(candidate.type))) {
            const matches = activeMerges.some((merge) => (row.successors.length === 1
