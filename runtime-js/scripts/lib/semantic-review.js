...
import { materialHash, materialTableMarkdown, readMaterialFile, readRepresentationContext, degradedPacketMaterialView, indeterminateClaimMaterialPreview, representationUseSubjectJson, representationReviewView, validateMaterialUseInput, validateRepresentationUse, representationUseNeedsReview, materialFeatureAvailable, assertMaterialReviewUpheld, } from './source-representation.js';
...
export function semanticRelationRow(proposal) {
...
export function degradedPacketBinding(runFormatVersion, returned, outputIndex) {
    requireSemantic(hasRunCapability(runFormatVersion, 'orchestrator-work-transitions'), 'SEM_COMPATIBILITY', 'degraded-packet', 'orchestrator-work-transitions required');
    ...
    const candidate = returned.packets[outputIndex];
    ...
    return { output_binding: { kind: 'degraded-packet', source_id: String(returned.source_id),
            ...
            criterion: candidate.criterion }, material_use: structuredClone(candidate.material_use),
        entry: structuredClone(returned.semantic_units.find((entry) => entry.output_kind === 'packet-candidate' && entry.output_index === outputIndex)) };
}
export function semanticDegradedMaterialViews(model, output, producerBindingHash, materialUse, context = readRepresentationContext(model)) {
    const view = parseSemanticJson(degradedPacketMaterialView(model, context, output.source_id, producerBindingHash, materialUse));
...
export function indeterminateClaimBinding(model, returned, outputIndex, reservedClaimId) {
    requireSemantic(hasRunCapability(model.manifest.runFormatVersion, 'orchestrator-work-transitions'), 'SEM_COMPATIBILITY', 'indeterminate-claim', 'orchestrator-work-transitions required');
    ...
    const candidate = returned.claims[outputIndex];
    ...
    const packets = candidate.packets;
    ...
    return { output_binding: { kind: 'indeterminate-claim', output_selector: `claim-candidate:${outputIndex}`, output_index: outputIndex,
        ...
        material_use: structuredClone(use), entry: structuredClone(returned.semantic_units
            .find((entry) => entry.output_kind === 'claim-candidate' && entry.output_index === outputIndex)) };
}
export function semanticIndeterminateClaimMaterialViews(model, output, producerBindingHash, materialUse, context = readRepresentationContext(model)) {
...
function subjectOutput(value) {
    ...
    if (value.kind === 'indeterminate-claim')
        requireSemantic(ordinal(value.output_index)
    ...
    if (value.kind === 'degraded-packet') {
        ...
        text(value.degraded_source_locator, 'degraded_source_locator');
        text(value.degradation_reason, 'degradation_reason');
    }
}
...
export function validateSemanticSubjectShape(value) {
    ...
    requireSemantic(!['degraded-packet', 'indeterminate-claim'].includes(value.subject_kind)
        || hasRunCapability(String(value.run_binding.run_format_version), 'orchestrator-work-transitions'), 'SEM_COMPATIBILITY', 'degraded-packet', 'orchestrator-work-transitions required');
    ...
    requireSemantic(subject.subject_kind !== 'indeterminate-claim' || subject.material_use?.use_state === 'CANNOT_DETERMINE', 'SEM_STATE', 'indeterminate-claim', 'nonaffirmative material declaration required');
...
function validateDegradedSemantics(semantics, anchors) {
    requireSemantic(semantics.atomicity === 'CANNOT_DETERMINE' && anchors.length === 0 && semantics.units.length === 0
        && semantics.contexts.length === 0 && semantics.couplings.length === 0 && semantics.relation_proposals.length === 0, 'SEM_STATE', 'degraded-packet', 'indeterminate material findings only; no affirmative or exact evidence');
}
...
export function claimProposalModel(model, output) {
...
export function useRowFromSubject(value) {
...
function subjectSources(subject, material) {
    ...
    if (subject.output_binding.kind === 'degraded-packet')
        ids.push(subject.output_binding.source_id);
...
export function validateSemanticWorkingAmbiguity(model, s, value) {
    const a = value;
...
function validateProducerSelection(subject, returned, tuple) {
    ...
    if (output.kind === 'packet-group') {
        ...
        requireSemantic(hasRunCapability(subject.run_binding.run_format_version, 'orchestrator-work-transitions')
            ...
            && output.degradation_reason === candidate.degradation_reason && output.criterion === candidate.criterion, 'SEM_SUBJECT', 'degraded-packet', 'original degraded producer selection differs');
        validateDegradedSemantics(subject.semantics, subject.anchors);
    }
        ...
        if (hasRunCapability(subject.run_binding.run_format_version, 'orchestrator-work-transitions')) {
            requireSemantic((output.kind === 'indeterminate-claim') === (subject.material_use?.use_state === 'CANNOT_DETERMINE'), 'SEM_SUBJECT', 'claim candidate', 'original nonaffirmative selector requires its dedicated binding');
            if (output.kind === 'indeterminate-claim')
                requireSemantic(output.output_index === tuple.output_index
...
export function validateSemanticRun(model) {
    ...
    for (const row of ledger.subjects) {
        ...
        if (hasRunCapability(model.manifest.runFormatVersion, 'orchestrator-work-transitions')) {
            requireSemantic(![...producers.values()].some((prior) => semanticProducerBinding(prior) === subject.producer_binding_hash), 'SEM_ACCOUNTING', row.semantic_id, 'duplicate semantic subject for original producer selector');
        }
...
export function planSemanticWrite(options) {
    ...
    if (stage === 'S4' && operation === 'admit' && hasRunCapability(model.manifest?.runFormatVersion || '', 'duplicate-overlap-review'))
        validateDuplicateRun(options.proposedModel);
...
function producerView(model, role, stage, selections, retained, completionHistory) {
    ...
    for (const entry of selections) {
        ...
        if (entry.purpose === 'ambiguity-context') {
            ...
            requireSemantic(stage === 'S4' && role === 'normalizer' && entry.selector === 'json:/proposal/successor_request', 'SEM_ISOLATION', entry.path, 'normalizer receives only the reviewed successor request');
        }
    ...
    if (stage === 'S4' && hasRunCapability(model.manifest?.runFormatVersion || '', 'duplicate-overlap-review')) {
        ...
        requireSemantic(selections.filter((entry) => entry.path === duplicatePath('subjects', duplicate.proposal_id)
            && entry.selector === 'json:/proposal/successor_request').length === 1, 'SEM_ISOLATION', 'S4', 'exact reviewed request must actually be shown');
        requireSemantic(semanticJson(context.origin_unit_refs) === semanticJson(duplicate.proposal.member_semantic_refs.flatMap((m) => m.unit_refs)), 'SEM_REFERENCE', 'S4', 'normalizer cannot alter reviewed comparison membership');
    }
    ...
    try {
        ...
        if (role === 'verifier-l2s') {
            ...
            requireSemantic(hasRunCapability(runFormatVersion, 'orchestrator-work-transitions')
                || !value.field_reviews.some((row) => INDETERMINATE_CLAIM_REVIEW_PATHS.includes(row.field_path)), 'SEM_COMPATIBILITY', 'indeterminate-claim', 'orchestrator-work-transitions required');
            if (context?.subject) {
