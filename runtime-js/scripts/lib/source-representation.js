...
export function indeterminateClaimMaterialPreview(model, context, basis) {
    requireMaterial(hasRunCapability(model.manifest?.runFormatVersion || '', 'orchestrator-work-transitions'), 'FORMAT', 'indeterminate-claim', 'run_format', 'orchestrator-work-transitions required');
    requireMaterial([basis.producer_binding_hash, basis.proposal_digest].every((hash) => /^sha256:[0-9a-f]{64}$/u.test(hash)), 'USE_CLOSURE', 'indeterminate-claim', 'binding', 'exact producer and proposal digests required');
    ...
    requireMaterial(sources.length > 0 && JSON.stringify(sources) === JSON.stringify(basis.source_ids), 'USE_CLOSURE', 'indeterminate-claim', 'source_ids', 'exact ordered packet source basis required');
    const input = validateMaterialUseInput(basis.material_use);
    requireMaterial(input.use_state === 'CANNOT_DETERMINE', 'USE_CLOSURE', 'indeterminate-claim', 'use_state', 'retained nonaffirmative declaration required');
    ...
    return materialReviewView(model, context, input, sources, basis.packet_ids, subject, `indeterminate-claim-material-preview:${materialHash(JSON.stringify(subject))}`);
}
