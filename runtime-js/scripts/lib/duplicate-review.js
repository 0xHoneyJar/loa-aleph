...
import { runLogEvents, mdLineSpan, sourceFilePath, reachedState } from './check-helpers.js';
import { parseTables, envelopeSection } from './markdown.js';
import { hasRunCapability, forwardExecutionIdentityProblems, walkFiles } from './run-model.js';
import { parseLineage, lineageCurrentClaimIds } from './lineage.js';
import { relationReviewSubjectJson, RELATION_REVIEW_SUBJECT_FORMAT } from './relations.js';
...
import { materialHash, materialTableMarkdown, readMaterialFile, readRepresentationContext, validateMaterialUseInput, planRepresentationUseWrite, validateMaterialPlanIdentity, validateRepresentationRun, MATERIAL_FEATURES } from './source-representation.js';
import { parseStrictJson, validateJudgmentRationale } from './worker-return-contract.js';
import { canonicalJsonBytes, bundleLockBytes, resealBundleLock } from './bundle-format.js';
import { parseStructuredVerifierRecord, INTERNAL_AMBIGUITY_REVIEW_SUBJECT_FORMAT } from './internal-ambiguity.js';
import { semanticJson, parseSemanticJson, semanticId, semanticPacketBasis, semanticRunBinding, semanticOriginProjection, semanticSubjectPath, parseSemanticLedger, SEMANTIC_PATH, semanticMaterialViews, completeSemanticAnchors, selectSemanticCorePart, semanticRelationRow, validateSemanticSubject, validateSemanticRun, SEMANTIC_EXECUTIONS, validateSemanticAcceptedBindings, validateSemanticWorkingAmbiguity, planSemanticWrite, validateSemanticPlan, validateSemanticResult, } from './semantic-review.js';
...
export const DUPLICATE_DIMENSIONS = ['proposition', 'conditions', 'qualifiers', 'scope', 'modality', 'attribution',
    ...
    'material', 'ambiguity', 'lineage', 'relations', 'context'];
export const DUPLICATE_OUTCOMES = ['duplicate', 'overlap', 'distinct', 'CANNOT_DETERMINE'];
export const DUPLICATE_VERDICTS = ['upheld', 'refuted', 'cannot-determine'];
export const DUPLICATE_REASONS = ['reviewed-duplicate', 'reviewed-nonduplicate', 'unresolved-equivalence',
    'unresolved-origin', 'refuted-proposal', 'successor-not-preserved', 'withdrawn-reservation', 'changed-prerequisite'];
...
export function requireDuplicate(condition, token, path, why) {
    if (!condition)
        throw new Error(`${token} ${path}: ${why}`);
}
function object(v) { return v !== null && typeof v === 'object' && !Array.isArray(v); }
const obj = object;
function keys(v, expected, path) {
    requireDuplicate(object(v) && semanticJson(Object.keys(v)) === semanticJson(expected), 'DUP_FORMAT', path, 'exact keys and order required');
}
function array(v, path) { requireDuplicate(Array.isArray(v), 'DUP_FORMAT', path, 'array required'); }
function text(v, path) { requireDuplicate(typeof v === 'string' && v.trim(), 'DUP_FORMAT', path, 'nonempty text required'); }
function hash(v, path) { requireDuplicate(typeof v === 'string' && /^sha256:[a-f0-9]{64}$/u.test(v), 'DUP_FORMAT', path, 'lowercase SHA-256 required'); }
function oneOf(v, values, path) { requireDuplicate(values.includes(v), 'DUP_ENUM', path, 'unknown enum'); }
function strings(v, path, nonempty = false) {
    array(v, path);
    requireDuplicate((!nonempty || v.length > 0) && v.every((x) => typeof x === 'string' && x.length) && new Set(v).size === v.length, 'DUP_REFERENCE', path, 'unique nonempty strings required');
}
function subset(v, basis, path, nonempty = false) {
    strings(v, path, nonempty);
    requireDuplicate(v.every((x, i) => basis.includes(x) && (i === 0 || basis.indexOf(x) > basis.indexOf(v[i - 1]))), 'DUP_REFERENCE', path, 'ordered subset required');
}
function equal(a, b, token, path) { requireDuplicate(semanticJson(a) === semanticJson(b), token, path, 'exact ordered binding differs'); }
function unique(values) { const seen = new Set(); return values.filter((v) => { const key = semanticJson(v); if (seen.has(key))
    return false; seen.add(key); return true; }); }
function decode(v) { return v.replace(/&#10;/gu, '\n').replace(/&#13;/gu, '\r').replace(/&#124;/gu, '|').replace(/&amp;/gu, '&'); }
function ids(v) { return v === 'none' ? [] : v.split(',').map((x) => x.trim()); }
function integer(v) { return typeof v === 'number' && Number.isSafeInteger(v) && v >= 0 && !Object.is(v, -0); }
function rationale(v) { text(v, 'rationale'); const errors = []; validateJudgmentRationale(v, 'rationale', errors); requireDuplicate(!errors.length, 'DUP_REVIEW', 'rationale', errors.join('; ')); }
export function duplicateJson(v) {
    try {
        return semanticJson(v);
    }
    catch (error) {
        throw new Error(`DUP_FORMAT ${error instanceof Error ? error.message : String(error)}`);
    }
}
export function parseDuplicateJson(bytes) {
    try {
        return parseSemanticJson(bytes);
    }
    catch (error) {
        throw new Error(`DUP_FORMAT ${error instanceof Error ? error.message : String(error)}`);
    }
}
export function duplicatePath(kind, id) {
    return `verification/harness/duplicate-${kind}/${id}.json`;
}
export function duplicatePointer(value, pointer) {
...
function refs(v, basis, path, nonempty = false) {
    strings(v, path, nonempty);
    for (const ref of v) {
        requireDuplicate(ref.startsWith('/'), 'DUP_REFERENCE', path, 'basis pointer required');
        if (basis)
            duplicatePointer(basis, ref);
    }
}
function anchorRefs(v, basis) {
    array(v, 'anchor_refs');
    ...
    for (const ref of v) {
        ...
        if ('semantic_id' in ref) {
            ...
            requireDuplicate(semanticId(ref.semantic_id, 'SEM') && typeof ref.anchor_id === 'string' && /^A[1-9]\d*$/u.test(ref.anchor_id), 'DUP_REFERENCE', 'anchor_ref', 'SEM/anchor identity required');
            if (basis)
                requireDuplicate(basis.semantic_projections.some((s) => s.semantic_id === ref.semantic_id && s.anchors.some((a) => a.anchor_id === ref.anchor_id)), 'DUP_REFERENCE', 'anchor_ref', 'missing semantic anchor');
        }
        else {
            keys(ref, ['source_id', 'locator', 'start_byte', 'end_byte', 'selection_hash'], 'inspection anchor');
            text(ref.source_id, 'source_id');
            text(ref.locator, 'locator');
            hash(ref.selection_hash, 'selection_hash');
            ...
            if (basis)
                requireDuplicate(basis.inspection_anchors.some((a) => a.source_id === ref.source_id && a.locator === ref.locator
                    && a.start_byte === ref.start_byte && a.end_byte === ref.end_byte && a.selection_hash === ref.selection_hash), 'DUP_REFERENCE', 'anchor_ref', 'not in inspection basis');
        }
    }
}
function requestedContext(v, basis) {
    array(v, 'requested_context');
    for (const r of v) {
        keys(r, ['source_id', 'locator', 'purpose'], 'requested_context');
        text(r.source_id, 'source_id');
        text(r.locator, 'locator');
        ...
        if (basis)
            requireDuplicate(basis.sources.some((s) => s.source_id === r.source_id)
                || basis.semantic_projections.some((s) => s.anchors.some((a) => a.source_id === r.source_id))
                || basis.inspection_anchors.some((a) => 'source_id' in a && a.source_id === r.source_id), 'DUP_REFERENCE', 'requested_context', 'request cannot leave frozen comparison context');
    }
}
function materialRefs(v, basis) {
    ...
    for (const r of v) {
        ...
        requireDuplicate(requirement ? integer(r.requirement_index) : typeof r.limitation_id === 'string' && /^(REP|OBJ|ASC)-(?=\d*[1-9])\d{4,}$/u.test(r.limitation_id), 'DUP_REFERENCE', 'material_ref', 'existing requirement or limitation required');
        if (basis) {
            ...
            requireDuplicate(Array.isArray(selections) && (requirement ? Number(r.requirement_index) < selections.length : selections.includes(r.limitation_id)), 'DUP_REFERENCE', 'material_ref', 'use selection absent');
        }
    }
}
function findings(value, basis) {
    ...
    value.forEach((f, i) => {
        ...
        oneOf(f.dimension, DUPLICATE_DIMENSIONS, 'dimension');
        refs(f.input_refs, basis, 'input_refs', true);
        anchorRefs(f.anchor_refs, basis);
        materialRefs(f.material_refs, basis);
        text(f.missing, 'missing');
        requestedContext(f.requested_context, basis);
    });
}
export function duplicatePairs(members) {
    return members.flatMap((a, i) => members.slice(i + 1).map((b) => ({ a, b })));
}
function contradictions(v, members, distinctionIds, basis) {
    ...
    for (const p of v) {
        ...
        if (pairs) {
            const index = pairs.indexOf(`${p.a}/${p.b}`);
            requireDuplicate(index > previous, 'DUP_REFERENCE', 'pair', 'exact member pair order required');
            previous = index;
        }
        strings(p.distinction_refs, 'distinction_refs', true);
        if (distinctionIds)
            requireDuplicate(p.distinction_refs.every((r) => distinctionIds.includes(r)), 'DUP_REFERENCE', 'pair', 'missing distinction');
        anchorRefs(p.anchor_refs, basis);
        requireDuplicate(p.anchor_refs.length > 0, 'DUP_REFERENCE', 'pair', 'anchors required');
        text(p.why, 'why');
    }
}
function occurrenceShape(v) {
    keys(v, ['source_id', 'source_hash', 'packet_id', 'evidence_key', 'fragment_order', 'locator', 'fragment_hash', 'start_byte', 'end_byte'], 'occurrence');
    for (const key of ['source_id', 'packet_id', 'evidence_key', 'locator'])
        text(v[key], key);
    hash(v.source_hash, 'source_hash');
    hash(v.fragment_hash, 'fragment_hash');
    requireDuplicate(integer(v.fragment_order) && v.fragment_order > 0 && integer(v.start_byte) && integer(v.end_byte) && v.end_byte > v.start_byte, 'DUP_EVIDENCE', 'occurrence', 'exact positive fragment/interval required');
}
function provenanceShape(v) {
    keys(v, ['packet_ids', 'source_ids', 'occurrences', 'member_occurrences'], 'provenance_union');
    strings(v.packet_ids, 'packet_ids', true);
    strings(v.source_ids, 'source_ids', true);
    array(v.occurrences, 'occurrences');
    array(v.member_occurrences, 'member_occurrences');
    ...
    for (const m of v.member_occurrences) {
        ...
        text(m.claim_id, 'claim_id');
        subset(m.occurrence_keys, v.occurrences.map(semanticJson), 'occurrence_keys', true);
        array(m.unit_occurrences, 'unit_occurrences');
        for (const u of m.unit_occurrences) {
            keys(u, ['unit_ref', 'anchor_refs', 'occurrence_keys'], 'unit_occurrence');
            text(u.unit_ref, 'unit_ref');
            anchorRefs(u.anchor_refs);
            subset(u.occurrence_keys, m.occurrence_keys, 'unit occurrence keys', true);
        }
    }
}
export function validateDuplicateProposal(value, basis, fields) {
    ...
    requireDuplicate(Array.isArray(value.member_ids) && new Set(value.member_ids).size === value.member_ids.length, 'DUP_REFERENCE', 'member_ids', 'unique members required');
    ...
    value.member_semantic_refs.forEach((m, i) => {
        ...
        requireDuplicate(m.claim_id === value.member_ids[i] && semanticId(m.semantic_id, 'SEM'), 'DUP_REFERENCE', 'member_semantic_refs', 'member/SEM mismatch');
        hash(m.subject_digest, 'subject_digest');
        strings(m.unit_refs, 'unit_refs', true);
    ...
    oneOf(value.outcome, DUPLICATE_OUTCOMES, 'outcome');
    ...
    requireDuplicate(value.review_mode !== 'unresolved-record' || value.outcome === 'CANNOT_DETERMINE' && value.treatment === 'keep-separate', 'DUP_STATE', 'review_mode', 'unresolved-record preserves uncertainty only');
    ...
    proposal.distinctions.forEach((d, i) => {
        ...
        oneOf(d.dimension, DUPLICATE_DIMENSIONS, 'dimension');
        subset(d.member_ids, proposal.member_ids, 'distinction members', true);
        refs(d.input_refs, basis, 'input_refs', true);
        oneOf(d.treatment, ['retained', 'collapsible', 'CANNOT_DETERMINE'], 'treatment');
        ...
        if (historical)
            requireDuplicate(d.input_refs.every((ref) => /^\/(?:occurrences|lineage_context|sources)(?:\/|$)/u.test(ref))
                && d.treatment === 'retained' && d.retained_at === 'occurrence-history', 'DUP_STATE', d.distinction_id, 'occurrences and historical identities cannot collapse');
        else
            requireDuplicate(d.retained_at !== 'occurrence-history', 'DUP_STATE', d.distinction_id, 'semantic field cannot disappear into occurrence history');
        ...
        requireDuplicate(d.treatment !== 'CANNOT_DETERMINE' || proposal.unresolved_findings.some((f) => f.dimension === d.dimension && d.input_refs.some((r) => f.input_refs.includes(r))), 'DUP_STATE', d.distinction_id, 'unknown distinction requires linked finding');
        anchorRefs(d.content_anchor_refs, basis);
        refs(d.context_refs, basis, 'context_refs');
        text(d.explanation, 'explanation');
        if (fields)
            subset(d.input_refs, fields.filter((f) => f.dimension === d.dimension).map((f) => f.field_ref), 'distinction field coverage', true);
    });
    for (const dimension of DUPLICATE_DIMENSIONS)
        requireDuplicate(proposal.distinctions.some((d) => d.dimension === dimension), 'DUP_ACCOUNTING', dimension, 'all seventeen dimensions required');
    if (fields)
        for (const f of fields)
            requireDuplicate(proposal.distinctions.some((d) => d.dimension === f.dimension && d.input_refs.includes(f.field_ref)), 'DUP_ACCOUNTING', f.field_ref, 'missing declared field coverage');
    ...
    refs(origin.basis_refs, basis, 'origin basis', true);
    subset(origin.unresolved_finding_refs, proposal.unresolved_findings.map((f) => f.finding_id), 'origin findings');
    ...
    array(origin.occurrence_groups, 'occurrence_groups');
    const occurrenceKeys = proposal.provenance_union.occurrences.map(semanticJson), grouped = [];
    for (const g of origin.occurrence_groups) {
        keys(g, ['occurrence_keys', 'basis_refs'], 'origin group');
        subset(g.occurrence_keys, occurrenceKeys, 'origin occurrences', true);
        refs(g.basis_refs, basis, 'origin group basis', true);
        grouped.push(...g.occurrence_keys);
    }
    requireDuplicate(grouped.length === occurrenceKeys.length && new Set(grouped).size === occurrenceKeys.length && occurrenceKeys.every((k) => grouped.includes(k)), 'DUP_ACCOUNTING', 'origin groups', 'exact occurrence partition required');
    if (proposal.treatment === 'keep-separate')
        requireDuplicate(proposal.representative === null && proposal.successor_request === null, 'DUP_STATE', 'successor', 'keep-separate requires null successor');
    else {
        ...
        requireDuplicate(r.wording_basis === 'selected-member' ? r.basis_member_ids.length === 1 : r.basis_member_ids.length >= 2, 'DUP_REFERENCE', 'representative', 'wording basis cardinality differs');
        ...
        oneOf(s.lineage_type, ['duplicate', 'merge'], 'lineage_type');
        text(s.proposed_claim, 'proposed_claim');
        ...
        refs(s.semantic_content_refs, basis, 'semantic content refs', true);
        validateMaterialUseInput(s.material_use);
    }
    if (basis) {
        ...
        equal(proposal.member_semantic_refs, basis.members.map((m, i) => ({ claim_id: m.claim_id, semantic_id: m.semantic_id,
            subject_digest: m.semantic_subject_digest, unit_refs: basis.semantic_projections[i].semantics.units.map((u) => `${m.semantic_id}/${u.unit_id}`) })), 'DUP_SUBJECT', 'member SEM refs');
...
export function validateDuplicateResult(value, subject) {
    ...
    requireDuplicate(value.format === DUPLICATE_RESULT_FORMAT, 'DUP_FORMAT', 'result', 'wrong format');
    hash(value.subject_digest, 'subject_digest');
    oneOf(value.verdict, DUPLICATE_VERDICTS, 'verdict');
    oneOf(value.assessed_outcome, DUPLICATE_OUTCOMES, 'assessed_outcome');
    const basis = subject?.comparison_basis;
    findings(value.unresolved_findings, basis);
    strings(value.attacks_tried, 'attacks_tried', true);
    ...
    for (const key of ['dimension_reviews', 'distinction_reviews', 'pair_reviews'])
        array(value[key], key);
    const result = value;
    ...
    result.dimension_reviews.forEach((r, i) => {
        ...
        oneOf(r.verdict, DUPLICATE_VERDICTS, 'verdict');
        refs(r.input_refs, basis, 'dimension inputs', true);
        anchorRefs(r.anchor_refs, basis);
        materialRefs(r.material_refs, basis);
        text(r.explanation, 'explanation');
        if (subject)
            equal(r.input_refs, subject.comparison_fields.filter((f) => f.dimension === r.dimension).map((f) => f.field_ref), 'DUP_REVIEW', r.dimension);
        requireDuplicate(r.verdict !== 'cannot-determine' || result.unresolved_findings.some((f) => f.dimension === r.dimension), 'DUP_REVIEW', r.dimension, 'unknown dimension requires finding');
    });
    result.distinction_reviews.forEach((r, i) => {
        ...
        oneOf(r.verdict, DUPLICATE_VERDICTS, 'verdict');
        refs(r.input_refs, basis, 'distinction inputs', true);
        text(r.explanation, 'explanation');
        if (subject) {
            ...
            requireDuplicate(r.verdict !== 'cannot-determine' || result.unresolved_findings.some((f) => f.dimension === d.dimension && f.input_refs.some((p) => r.input_refs.includes(p))), 'DUP_REVIEW', r.distinction_id, 'unknown distinction requires linked finding');
        }
    });
    ...
    result.pair_reviews.forEach((r, i) => {
        ...
        text(r.a, 'a');
        text(r.b, 'b');
        requireDuplicate(r.a !== r.b, 'DUP_REFERENCE', 'pair', 'different members required');
        oneOf(r.verdict, DUPLICATE_VERDICTS, 'verdict');
        strings(r.distinction_refs, 'distinction_refs', true);
        refs(r.origin_basis_refs, basis, 'origin_basis_refs', true);
        text(r.explanation, 'explanation');
        if (subject) {
    ...
    if (subject) {
        ...
        requireDuplicate(result.distinction_reviews.length === subject.proposal.distinctions.length && result.pair_reviews.length === pairs.length, 'DUP_REVIEW', 'result', 'exact distinction/pair coverage required');
        requireDuplicate(result.verdict !== 'upheld' || result.assessed_outcome === subject.proposal.outcome
            && (result.assessed_outcome !== 'CANNOT_DETERMINE' || subject.proposal.review_mode === 'unresolved-record'), 'DUP_REVIEW', 'assessed_outcome', 'upheld assessment differs from proposal');
    }
}
/** This aggregation preserves round-one indeterminacy; it is not a vote. */
export function duplicateQuorum(rounds) {
    ...
    if (!second?.result)
        return { complete: false, verdict: null, review_ids: rounds.map((r) => r.assignment.review_id) };
    requireDuplicate(second.assignment.subject_digest === first.assignment.subject_digest && second.assignment.proposal_id === first.assignment.proposal_id, 'DUP_SUBJECT', 'round two', 'identical sealed subject required');
...
function claimProjection(model, id) {
...
function admittedSemantic(model, id) {
    const ledger = parseSemanticLedger(readMaterialFile(model.runDir, SEMANTIC_PATH).toString('utf8'));
    const candidates = ledger.resolutions.filter((r) => r.outcome === 'admitted').map((r) => parseSemanticJson(readMaterialFile(model.runDir, semanticSubjectPath(r.semantic_id))))
    ...
    try {
        validateSemanticSubject(candidates[0], model);
    }
    catch (error) {
        throw new Error(`DUP_SUBJECT ${id}: admitting SEM or its exact bound inputs changed: ${error instanceof Error ? error.message : String(error)}`);
    }
    return candidates[0];
}
...
function packetOccurrences(model, packets) {
...
export function duplicateProvenanceUnion(basis) {
    ...
    const source_ids = [...new Set(packet_ids.map((id) => {
            ...
            requireDuplicate(p, 'DUP_EVIDENCE', id, 'union packet absent');
            return p.packet.source_id;
        }))];
    ...
    const member_occurrences = basis.members.map((m, i) => {
        ...
        equal(m.claim_projection.sources, [...new Set(m.claim_projection.packets.map((id) => basis.packet_basis.find((p) => p.packet_id === id).packet.source_id))], 'DUP_ACCOUNTING', m.claim_id);
...
export function duplicateComparisonFields(basis) {
    const fields = [];
    const add = (field_ref, dimension, member) => {
        duplicatePointer(basis, field_ref);
        const previous = fields.find((f) => f.field_ref === field_ref && f.dimension === dimension);
        if (previous) {
            if (!previous.member_ids.includes(member))
                previous.member_ids.push(member);
        }
        else
            fields.push({ field_ref, dimension, member_ids: [member] });
    };
    for (const [i, member] of basis.members.entries())
        for (const dimension of DUPLICATE_DIMENSIONS) {
            ...
            if (dimension === 'proposition' || ['conditions', 'qualifiers', 'scope', 'modality', 'attribution', 'comparator', 'metric', 'claim_roles'].includes(dimension)) {
                for (const [u, unit] of s.units.entries()) {
                    const pointer = `${root}/units/${u}/${dimension}`;
                    add(pointer, dimension, member.claim_id);
                    if (dimension !== 'proposition') {
                        const facet = unit[dimension];
                        if (object(facet) && Array.isArray(facet.items))
                            facet.items.forEach((_, j) => add(`${pointer}/items/${j}`, dimension, member.claim_id));
                    }
                }
                if (!s.units.length)
                    add(`${root}/units`, dimension, member.claim_id);
            }
            else if (dimension === 'result-interpretation') {
                s.units.forEach((_, j) => add(`${root}/units/${j}/claim_roles`, dimension, member.claim_id));
                add(`${root}/couplings`, dimension, member.claim_id);
                s.couplings.forEach((_, j) => add(`${root}/couplings/${j}`, dimension, member.claim_id));
            }
            else if (dimension === 'source-occurrence' || dimension === 'support-origin') {
                basis.occurrences.forEach((o, j) => { if (member.claim_projection.packets.includes(o.packet_id))
                    add(`/occurrences/${j}`, dimension, member.claim_id); });
                if (!basis.occurrences.length)
                    add('/occurrences', dimension, member.claim_id);
                if (dimension === 'support-origin')
                    basis.sources.forEach((source, j) => {
                        if (member.claim_projection.sources.includes(source.source_id))
                            add(`/sources/${j}`, dimension, member.claim_id);
                    });
            }
            else {
                const collections = {
                        ...
                        collection.forEach((v, j) => {
                            ...
                            if (dimension === 'material' && object(v) && object(v.use_subject)) {
                                for (const key of ['requirements', 'limitation_refs']) {
                                    ...
                                    v.use_subject[key].forEach((_, k) => add(`${pointer}/${j}/use_subject/${key}/${k}`, dimension, member.claim_id));
                                }
                            }
                        });
                }
...
function referenced(model, reference) {
...
function selectedBytes(model, entry) {
    const bytes = readMaterialFile(model.runDir, entry.path);
    if (entry.selector.startsWith('json:'))
        return Buffer.from(semanticJson(duplicatePointer(parseStrictJson(bytes, true), entry.selector.slice(5))));
    ...
    const tables = parseTables(bytes.toString('utf8')).filter((t) => lines.slice(0, t.line - 1).reverse().find((line) => /^#{1,6} /u.test(line))?.replace(/^#{1,6} /u, '').replace(/\r$/u, '') === row[1]);
...
function definition(model, id) {
    if (id.startsWith('CC-'))
        return { kind: 'CC', id, projection: claimProjection(model, id) };
...
function lineageProjection(model, row) {
...
export function buildComparisonBasis(model, memberIds, historical) {
    const current = model.claims.map((r) => r.values.claimId).filter((id) => lineageCurrentClaimIds(model).has(id));
    if (!historical)
        subset(memberIds, current, 'current comparison group', true);
    ...
    const lineages = parseLineage(model).rows, seen = new Set(), lineage_context = [];
    function ancestry(id) {
        if (seen.has(id))
            return;
        seen.add(id);
        for (const row of lineages.filter((r) => ids(r.values.successors).includes(id))) {
            if (!lineage_context.some((c) => c.lineage_id === row.values.lineageId))
                lineage_context.push(lineageProjection(model, row));
    ...
    const add = (path, selector, purpose) => {
        ...
        entry.digest = materialHash(selectedBytes(model, entry));
        const old = context_manifest.find((e) => e.path === path && e.selector === selector);
        if (old)
            requireDuplicate(old.digest === entry.digest, 'DUP_SUBJECT', path, 'conflicting selection');
        else
            context_manifest.push(entry);
    };
    ...
    const inspection_anchors = [];
    const inspect = (source_id, locator, start, end) => {
    ...
    for (const s of subjects) {
        for (const entry of s.context_manifest)
            add(entry.path, entry.selector, entry.purpose);
        ...
        for (const finding of s.semantics.unresolved_findings)
            for (const request of finding.requested_context) {
    ...
    for (const path of walkFiles(workingRoot).filter((p) => p.endsWith('.json'))) {
        ...
        if (!bytes.includes(Buffer.from(INTERNAL_AMBIGUITY_REVIEW_SUBJECT_FORMAT)))
            continue;
        const value = parseStrictJson(bytes);
        if (!object(value) || value.format !== INTERNAL_AMBIGUITY_REVIEW_SUBJECT_FORMAT)
            continue;
        ...
        if (ambiguity_context.some((c) => c.reference === reference)
            || historical && !historical.ambiguity_context.some((c) => c.reference === reference))
            continue;
        ...
        if (value.search_scope_kind === 'full-same-source')
            inspect(String(value.source_id), `L1-L${lines}`);
        else {
            for (const id of parseDuplicateJson(String(value.search_completion_ref))) {
        ...
        validateSemanticWorkingAmbiguity(model, { ...parent, anchors: [...parent.anchors, ...inspection_anchors] }, value);
        ambiguity_context.push({ kind: 'slice5-working-subject', reference, digest: materialHash(semanticJson(value)), projection: value });
    ...
    if (existsSync(join(model.runDir, DUPLICATE_PATH))) {
        ...
        for (const row of priorLedger.proposals.filter((r) => priorLedger.effects.some((e) => e.proposal_id === r.proposal_id))) {
            const prior = parseDuplicateJson(readMaterialFile(model.runDir, row.subject_path));
            if (!prior.proposal.member_ids.some((id) => seen.has(id)))
                continue;
            const selected = [{ path: row.subject_path, pointer: '/proposal/unresolved_findings', findings: prior.proposal.unresolved_findings },
                ...priorLedger.results.filter((r) => r.proposal_id === row.proposal_id).map((r) => ({
                    ...
                    findings: parseDuplicateJson(readMaterialFile(model.runDir, r.result_path)).unresolved_findings,
                }))];
            for (const entry of selected)
                for (const [index, finding] of entry.findings.entries()) {
                    ...
                    if (historical && !historical.ambiguity_context.some((c) => c.reference === reference))
                        continue;
    ...
    for (const row of semanticLedger.subjects) {
        const s = parseSemanticJson(readMaterialFile(model.runDir, row.subject_path));
        for (const [i, relation] of s.semantics.relation_proposals.entries()) {
            ...
            if (historical && !historical.relation_context.some((r) => r.reference === reference))
                continue;
            ...
            relation_context.push({ reference, digest: relation.review_subject_digest, subject: p,
                target_units: context.target_units, target_anchors: context.target_anchors,
            ...
            const j = s.relation_context.indexOf(context);
            add(row.subject_path, `json:/relation_context/${j}`, 'relation-context');
    ...
    for (const path of existsSync(acceptedRoot) ? walkFiles(acceptedRoot).filter((p) => p.endsWith('/raw.json')) : []) {
        ...
        if (!object(report) || report.result !== 'PASS')
            continue;
        const bytes = readMaterialFile(model.runDir, rawPath), returned = parseStrictJson(bytes, true);
        if (!object(returned) || !Array.isArray(returned.relation_proposals))
            continue;
        ...
        requireDuplicate(object(request) && request.kind === 'producer' && request.run_id === model.manifest.runId, 'DUP_ISOLATION', rawPath, 'retained relation producer request required');
        for (const [i, item] of returned.relation_proposals.entries()) {
            ...
            if (historical && !historical.relation_context.some((c) => c.reference === reference))
                continue;
            const fields = ['owner_stage', 'family', 'type', 'source_kind', 'source_id', 'target_kind', 'target_id', 'target_source_id',
                'target_locator', 'target_span_hash', 'record_state', 'null_reason', 'basis_packet_ids', 'proposed_by'];
            const projection = { subject: { format: RELATION_REVIEW_SUBJECT_FORMAT, ...Object.fromEntries(fields.map((key) => [key, item[key]])) },
                review_subject_digest: item.review_subject_digest, material_use: item.material_use };
            ...
            const targetPackets = unique(target_units.flatMap((u) => u.kind === 'PKT' ? [u.id] : u.projection.packets));
            ...
            for (const target of target_units.filter((u) => u.kind === 'CC')) {
                ...
                for (const entry of semantic.context_manifest)
                    add(entry.path, entry.selector, entry.purpose);
            }
            relation_context.push({ reference, digest: projection.review_subject_digest, subject: projection.subject,
                target_units: target_units, target_anchors, target_packet_context, material_use: projection.material_use });
            for (const field of [...fields, 'review_subject_digest', 'material_use'])
                add(rawPath, `json:${pointer}/${field}`, 'relation-context');
            for (const a of target_anchors)
                inspect(a.source_id, a.locator, a.start_byte, a.end_byte);
        }
    }
    for (const relation of relation_context)
        for (const requirement of relation.material_use.requirements) {
            requireDuplicate(material_views.some((view) => object(view.use_subject) && Array.isArray(view.use_subject.requirements)
                && view.use_subject.requirements.some((r) => semanticJson(r) === semanticJson(requirement))), 'DUP_EVIDENCE', relation.reference, 'required relation material view absent; retain missing context before review');
        }
    ...
    for (const [index, row] of model.claims.entries())
        if (memberIds.includes(row.values.claimId)) {
...
export function emptyDuplicateLedger() { return { discoveries: [], proposals: [], assignments: [], results: [], decisions: [], effects: [] }; }
export function duplicateLedgerMarkdown(ledger) {
    return `# Duplicate review\n\nduplicate_review_format: ${DUPLICATE_FORMAT}\n\n`
        + Object.keys(DUPLICATE_HEADERS).map((key) => `## ${key}\n\n${materialTableMarkdown([...DUPLICATE_HEADERS[key]], ledger[key])}`).join('\n');
}
export function parseDuplicateLedger(raw) {
    requireDuplicate((raw.match(/^duplicate_review_format: aleph-duplicate-review\/v1\r?$/gmu) || []).length === 1, 'DUP_FORMAT', DUPLICATE_PATH, 'exact single format marker required');
    ...
    for (const [i, key] of Object.keys(DUPLICATE_HEADERS).entries()) {
        ...
        ledger[key] = tables[i].rows.map((row) => {
            ...
            idSet.add(row.cells[0]);
            return Object.fromEntries(DUPLICATE_HEADERS[key].map((h, i) => [h, decode(row.cells[i])]));
        });
    }
    return ledger;
}
export function duplicateStage(model) {
    let stage = -1;
    ...
    if (existsSync(join(model.runDir, path))) {
        ...
        requireDuplicate(object(state) && object(state.identity) && object(state.execution)
            && state.run_id === model.manifest?.runId && state.identity.run_format_version === model.manifest?.runFormatVersion, 'DUP_STATE', path, 'retained execution identity differs');
        requireDuplicate(typeof state.execution.stage === 'string' && /^S(?:[0-9]|1[0-3])[ab]?$/u.test(state.execution.stage), 'DUP_STATE', path, 'invalid retained stage');
        stage = Number.parseInt(state.execution.stage.slice(1), 10);
    }
    for (const event of runLogEvents(model.runLog))
        if (/^S(?:[0-9]|1[0-3])[ab]?$/u.test(event.stage))
            stage = Math.max(stage, Number.parseInt(event.stage.slice(1), 10));
    return stage;
}
export function duplicateClosureHash(model) {
    ...
    requireDuplicate(match && event?.stage === 'S4' && /^closure_phase: S4-C1-relations-closed\r?$/mu.test(body), 'DUP_WINDOW', 'C1', 'exact additional field inside existing C1 event required');
    return match[1];
}
export function assertDuplicateWindow(model) {
    ...
    requireDuplicate(runLogEvents(model.runLog).some((e) => e.stage === 'S3' && /^(exit|closed|closure)\b/u.test(e.event)), 'DUP_WINDOW', 'S3', 'retained S3 closure required');
}
export const DUPLICATE_LENS = 'L3 — duplicate-versus-overlap refutation (1.8)';
export function duplicatePromptRequirements(task) {
...
function pinnedPrompts(model, task) {
    const identity = model.manifest.forwardIdentity, root = 'control/runtime/bundle';
    const raw = readMaterialFile(model.runDir, `${root}/bundle.lock.json`), lock = parseStrictJson(raw), sealed = resealBundleLock(lock);
    requireDuplicate(raw.equals(bundleLockBytes(lock)) && sealed.lock_digest === lock.lock_digest && sealed.bundle.digest === lock.bundle.digest
        && lock.bundle.digest === identity.bundleDigest && lock.core.tree_digest === identity.coreDigest && lock.checker_digest === identity.checkerDigest, 'DUP_SUBJECT', root, 'exact immutable run lock required');
...
export function validateDuplicateReviewerProfile(value, model) {
    if (model.manifest?.mode === 'manual')
        requireDuplicate(object(value)
            && semanticJson(Object.keys(value)) === semanticJson(['profile_id', 'profile_digest', 'role', 'model_identity']), 'DUP_ISOLATION', 'manual profile', 'exact four-key Q8-MANUAL profile required');
    keys(value, ['profile_id', 'profile_digest', 'role', 'model_identity'], 'reviewer_profile');
    requireDuplicate(value.role === 'verifier-l3' && model.manifest && forwardExecutionIdentityProblems(model.manifest).length === 0, 'DUP_ISOLATION', 'reviewer_profile', 'retained run identity and L3 role required');
    if (model.manifest.mode === 'manual') {
    ...
    requireDuplicate(efforts.includes(String(producer.effort)) && efforts.indexOf(String(mapping.effort)) >= efforts.indexOf(String(producer.effort)), 'DUP_ISOLATION', 'effort', 'reviewer effort below producer');
    ...
    requireDuplicate(m.context === mapping.context_policy && m.effort === mapping.effort && m.budget === mapping.budget_policy
        && m.cache === mapping.cache_policy && m.batch === mapping.batch_policy && m.immutable === true && m.fallback === false, 'DUP_ISOLATION', 'model', 'unpinned context/effort/fallback');
    for (const declaration of [identity.modelIds, identity.modelExecutionMapping]) {
        const mapping = parseStrictJson(declaration);
        requireDuplicate(object(mapping), 'DUP_ISOLATION', 'model mapping', 'object required');
        equal(mapping['verifier-l3'], m, 'DUP_ISOLATION', 'model mapping');
    }
}
export function duplicateProducerBinding(tuple) {
...
function retainedProducerDelivery(model, task, tuple, shown) {
    ...
    requireDuplicate(object(request) && request.kind === 'producer' && request.role === 'merge-judge' && request.stage === 'S4'
        ...
        && object(dispatch) && object(dispatch.receipt) && dispatch.receipt.call_id === callId && dispatch.receipt.context_id === tuple.context_id, 'DUP_ISOLATION', root, 'actual accepted merge-judge request, return and context required');
    ...
    requireDuplicate(Array.isArray(allowlist) && allowlist.length === 1 && object(allowlist[0])
        ...
        && readMaterialFile(model.runDir, `${root}/${allowlist[0].attachment_path}`).equals(shown), 'DUP_ISOLATION', root, 'actual producer attachment set differs');
}
export function duplicateSweepView(model, catalogue) {
...
export function buildDuplicateSubject(model, input) {
    assertDuplicateWindow(model);
    const basis = buildComparisonBasis(model, input.proposal.member_ids);
    const subject = { format: DUPLICATE_SUBJECT_FORMAT, proposal_id: input.proposal_id, owner_stage: 'S4',
...
export function validateDuplicateSubject(value, model, fresh = false) {
    ...
    const subject = value, basis = subject.comparison_basis;
    ...
    if (subject.proposal.treatment === 'keep-separate')
        requireDuplicate(subject.reservation === null, 'DUP_STATE', 'reservation', 'no reservation for separate claims');
    else {
        keys(subject.reservation, ['lineage_id', 'successor_id', 'lineage_type', 'predecessor_ids'], 'reservation');
        requireDuplicate(/^LIN-\d+$/u.test(String(subject.reservation.lineage_id)) && /^CC-\d+$/u.test(String(subject.reservation.successor_id)), 'DUP_REFERENCE', 'reservation', 'existing new successor/lineage namespaces required');
        equal(subject.reservation.predecessor_ids, subject.proposal.member_ids, 'DUP_REFERENCE', 'reserved predecessors');
        requireDuplicate(subject.reservation.lineage_type === subject.proposal.successor_request.lineage_type && !subject.proposal.member_ids.includes(subject.reservation.successor_id), 'DUP_REFERENCE', 'reservation', 'new identity and reviewed type required');
        if (fresh)
            requireDuplicate(!model.claims.some((r) => r.values.claimId === subject.reservation.successor_id)
                && !parseLineage(model).rows.some((r) => r.values.lineageId === subject.reservation.lineage_id), 'DUP_REFERENCE', 'reservation', 'new reservations cannot reuse admitted definitions');
    }
    if (fresh)
        assertDuplicateWindow(model);
}
export function duplicateAttachmentPaths(subject) {
    const paths = [duplicatePath('subjects', subject.proposal_id)];
    for (const m of subject.comparison_basis.material_views) {
        const view = m.view;
        for (const asset of (Array.isArray(view.assets) ? view.assets : []))
            if (typeof asset.locus === 'string' && typeof asset.bytes_base64 === 'string')
                paths.push(asset.locus);
    }
    return unique(paths).sort((a, b) => Buffer.compare(Buffer.from(a), Buffer.from(b)));
}
export function validateDuplicateAttachmentDelivery(subject, task, attachments) {
    ...
    for (const a of attachments) {
        if (a.path === duplicatePath('subjects', subject.proposal_id))
            requireDuplicate(a.bytes.equals(Buffer.from(semanticJson(subject))), 'DUP_SUBJECT', a.path, 'delivered subject changed');
        else {
            ...
            requireDuplicate(asset && typeof asset.bytes_base64 === 'string' && a.bytes.equals(Buffer.from(asset.bytes_base64, 'base64')), 'DUP_EVIDENCE', a.path, 'actual material asset differs');
        }
    }
}
export function duplicateAdmissionProblems(subject, verdict) {
    const p = subject.proposal, errors = [];
    if (verdict !== 'upheld')
        errors.push('round-one and DDR upheld required');
    if (p.review_mode !== 'proposal' || p.outcome !== 'duplicate' || p.treatment !== 'new-successor')
        errors.push('reviewed new duplicate successor required');
    if (p.contradiction_pairs.length || p.unresolved_findings.length)
        errors.push('contradiction or unresolved finding blocks absorption');
    if (p.distinctions.some((d) => d.treatment === 'CANNOT_DETERMINE' || d.retained_at === 'separate-claims'))
        errors.push('surviving separate or unknown distinction');
    if (p.origin_assessment.corroboration === 'CANNOT_DETERMINE' || p.origin_assessment.unresolved_finding_refs.length)
        errors.push('binary reviewed origin required');
    return errors;
}
function newDuplicateBasis(next, prior) {
    const content = (basis) => {
    ...
    if (semanticJson(content(next)) !== semanticJson(content(prior)))
        return true;
...
function discoveryCatalogue(model, claimPrefix, lineagePrefix) {
...
export function buildDuplicateDiscovery(model, input) {
    assertDuplicateWindow(model);
    ...
    const d = { format: DUPLICATE_DISCOVERY_FORMAT, discovery_id: input.discovery_id, run_binding: semanticRunBinding(model),
...
export function validateDuplicateDiscovery(value, model, fresh = false) {
    ...
    const d = value;
    strings(d.inventory_basis.claim_prefix_ids, 'claim prefix');
    strings(d.inventory_basis.lineage_prefix_ids, 'lineage prefix');
    requireDuplicate(d.inventory_basis.s3_seal_ref === s3SealRef(model), 'DUP_SUBJECT', 'inventory_basis', 'S3 seal differs');
    const lineages = parseLineage(model).rows;
    requireDuplicate(d.inventory_basis.lineage_prefix_digest === materialHash(semanticJson(lineages.slice(0, d.inventory_basis.lineage_prefix_ids.length).map((r) => r.cells.map(decode)))), 'DUP_SUBJECT', 'lineage prefix', 'historical lineage bytes differ');
    if (fresh) {
    ...
    for (const key of ['windows', 'candidates', 'sweep_refs', 'unresolved_findings'])
        array(value[key], key);
    const catalogueIds = d.catalogue.map((c) => c.claim_id), covered = new Set(), swept = new Set();
    ...
    for (const [i, w] of d.windows.entries()) {
        ...
        subset(w.member_ids, catalogueIds, 'window members', true);
        w.member_ids.forEach((id) => covered.add(id));
        ...
        requireDuplicate(duplicateProducerBinding(tuple) === w.producer_binding_hash && object(tuple) && tuple.output_kind === 'duplicate-discovery', 'DUP_SUBJECT', 'window', 'producer binding differs');
        ...
        array(returned.candidates, 'candidates');
        array(returned.unresolved_findings, 'unresolved_findings');
        rationale(returned.rationale);
        strings(returned.flags, 'flags');
        for (const c of returned.candidates) {
            ...
            strings(c.basis_refs, 'candidate basis', true);
            oneOf(c.signal, ['semantic-proposal', 'identical-text', 'shared-packet'], 'signal');
            returnedCandidates.push(c);
        }
        returnedFindings.push(...returned.unresolved_findings);
    }
    equal([...covered].sort(), [...catalogueIds].sort(), 'DUP_ACCOUNTING', 'global discovery coverage');
    requireDuplicate(d.producer_binding_hash === materialHash(semanticJson(d.windows.map((w) => ({ window_id: w.window_id, producer_binding_hash: w.producer_binding_hash })))), 'DUP_SUBJECT', 'discovery', 'aggregate producer binding differs');
    for (const [i, c] of d.candidates.entries()) {
        ...
        for (const ref of c.basis_refs) {
            if (ref.startsWith('/catalogue/'))
                duplicatePointer(d, ref);
            else
                referenced(model, ref);
        }
    }
    ...
    for (const c of returnedCandidates) {
        ...
        if (old)
            old.basis_refs = unique([...old.basis_refs, ...c.basis_refs]);
        else
    ...
    for (const sweep of d.sweep_refs) {
        ...
        sweep.window_member_ids.forEach((id) => swept.add(id));
        ...
        requireDuplicate(record.lens === 'L5' && record.stage === 'S4' && ver.path === `verification/harness/S4/${sweep.review_id}.md`, 'DUP_ISOLATION', sweep.review_id, 'independent L5 companion required');
        ...
        requireDuplicate(record.verdict === returned.verdict && (returned.verdict === 'refuted' || returned.flagged_pairs.length === 0), 'DUP_REVIEW', sweep.review_id, 'L5 result/companion differs');
        ...
        if (['agent', 'hybrid'].includes(model.manifest.mode)) {
            ...
            requireDuplicate(object(request) && request.role === 'verifier-l5' && request.stage === 'S4' && request.task_line === DUPLICATE_TASKS['contradiction-discovery']
                ...
                && readMaterialFile(model.runDir, `${root}/${request.allowlist[0].attachment_path}`).equals(shown), 'DUP_ISOLATION', 'L5', 'independent accepted sweep over exact view required');
        }
    }
    ...
    for (const f of d.unresolved_findings) {
        ...
        subset(f.member_ids, catalogueIds, 'finding members');
        text(f.missing, 'missing');
        requestedContext(f.requested_context);
        for (const request of f.requested_context)
            sourceSpan(model, request.source_id, request.locator);
    }
    requireDuplicate(returnedFindings.every((f) => d.unresolved_findings.some((r) => semanticJson(r) === semanticJson(f))), 'DUP_ACCOUNTING', 'discovery findings', 'producer finding lost');
...
export function deriveDuplicateExecution(model, subject, assignment, reference, producer) {
    ...
    if (evidence.path.startsWith('verification/harness/duplicate-process/') && evidence.path.endsWith('.md')) {
        ...
        requireDuplicate(record.target === `duplicate-review-subject:${digest}` && record.lens === 'L3' && record.stage === subject.owner_stage, 'DUP_REVIEW', evidence.path, 'static record binding differs');
    ...
    requireDuplicate(receipt.call_id === assignment.invocation_id && typeof receipt.context_id === 'string'
        ...
        && receipt.fresh_context === true && receipt.inherited_context === false && receipt.filesystem === 'bundle-read-only', 'DUP_ISOLATION', evidence.path, 'dispatch reused producer context or lost isolation');
    ...
    requireDuplicate(obj(request) && request.role === 'verifier-l3' && request.kind === 'refuter' && request.stage === subject.owner_stage
        ...
        && request.isolation.inherit_context === false && request.isolation.producer_context_id === producer.context_id, 'DUP_ISOLATION', requestPath, 'wrong run, role, kind, stage or context');
    requireDuplicate(Array.isArray(request.core_parts) && request.core_parts.length === subject.prompt_parts.length, 'DUP_SUBJECT', requestPath, 'exact sealed prompt parts required');
    request.core_parts.forEach((part, i) => {
        ...
        requireDuplicate(obj(part) && part.path === expected.path && part.selector === expected.selector && part.digest === expected.digest
            ...
            && materialHash(readMaterialFile(model.runDir, `control/worker-bundles/${assignment.invocation_id}/${part.materialized_path}`)) === expected.digest, 'DUP_SUBJECT', requestPath, 'actual delivered prompt bytes differ from sealed subject');
    });
    ...
    requireDuplicate(obj(invocation) && invocation.invocation_digest === record.invocation_digest
        ...
        && semanticJson(invocation.model_identity) === semanticJson(receipt.model_identity), 'DUP_ISOLATION', evidence.path, 'retained sealed invocation differs from actual dispatch');
    ...
    requireDuplicate(obj(invocation.host_capability_receipt)
        ...
        && record.host_capability_receipt_digest === materialHash(hostBytes), 'DUP_SUBJECT', evidence.path, 'dispatch host receipt differs');
    ...
    requireDuplicate(materialHash(nativeReturn) === record.structured_return_digest
        && semanticJson(parseStrictJson(nativeReturn, true)) === semanticJson(returned), 'DUP_SUBJECT', evidence.path, 'dispatched structured return differs from accepted raw return');
    requireDuplicate(readMaterialFile(model.runDir, duplicatePath('results', assignment.review_id)).equals(Buffer.from(semanticJson(returned))), 'DUP_SUBJECT', assignment.review_id, 'retained result differs from actual reviewer return');
    ...
    requireDuplicate(obj(validation) && validation.call_id === assignment.invocation_id && validation.result === 'PASS'
        && validation.raw_digest === materialHash(raw) && Array.isArray(validation.errors) && validation.errors.length === 0, 'DUP_REVIEW', assignment.review_id, 'actual accepted-return validation absent or mismatched');
    ...
    requireDuplicate(receipt.simulation === null && obj(record.host_evidence) && typeof record.event_stream_digest === 'string', 'DUP_ISOLATION', evidence.path, 'missing genuine retained host evidence');
    const state = parseStrictJson(readMaterialFile(model.runDir, 'control/run-state.json'));
    ...
    requireDuplicate(materialHash(events) === record.event_stream_digest && record.host_evidence.session_id === receipt.context_id, 'DUP_ISOLATION', evidence.path, 'native event stream/session differs');
    ...
    requireDuplicate(obj(host) && obj(host.runtime) && obj(host.runtime.claude) && obj(host.runtime.sandbox)
        ...
        && record.host_evidence.structured_output_digest === materialHash(canonicalJsonBytes(returned)), 'DUP_ISOLATION', evidence.path, 'native host/model/structured-output evidence differs');
...
export function validateDuplicateRun(model) {
    requireDuplicate(hasRunCapability(model.manifest?.runFormatVersion || '', 'duplicate-overlap-review'), 'DUP_COMPATIBILITY', 'run', 'active capability required');
    const entered = duplicateStage(model) >= 4, present = existsSync(join(model.runDir, DUPLICATE_PATH));
    if (!entered)
        requireDuplicate(!['process', 'discovery', 'subjects', 'assignments', 'results', 'effects'].some((kind) => existsSync(join(model.runDir, `verification/harness/duplicate-${kind}`))), 'DUP_WINDOW', 'S4', 'duplicate artifacts forbidden before structured S4 entry');
    const empty = { discoveries: 0, proposals: 0, assignments: 0, results: 0, decisions: 0, effects: 0, pending: [], executions: [] };
    if (!present) {
        requireDuplicate(!entered, 'DUP_FORMAT', DUPLICATE_PATH, 'ledger required on real S4 entry, including zero candidates');
        return empty;
    }
    ...
    const discoveries = new Map(), subjects = new Map();
    const assignments = new Map(), results = new Map();
    ...
    for (const row of ledger.proposals) {
        ...
        requireDuplicate(duplicateProducerBinding(tuple) === subject.producer_binding_hash && object(tuple) && tuple.output_kind === 'duplicate-proposal', 'DUP_SUBJECT', row.proposal_id, 'actual producer tuple differs');
        ...
        validateDuplicateProposal(returned.proposal, subject.comparison_basis, subject.comparison_fields);
        rationale(returned.rationale);
        strings(returned.flags, 'flags');
        ...
        subjects.set(row.proposal_id, subject);
        producers.set(row.proposal_id, tuple);
    }
    const invocationIds = new Set();
    for (const row of ledger.assignments) {
        const s = subjects.get(row.proposal_id);
        requireDuplicate(s && /^VER-\d+$/u.test(row.review_id) && row.assignment_path === duplicatePath('assignments', row.review_id), 'DUP_REVIEW', row.review_id, 'assignment lacks exact proposal/path');
        ...
        requireDuplicate(a.format === DUPLICATE_ASSIGNMENT_FORMAT && a.proposal_id === s.proposal_id && a.review_id === row.review_id
            ...
            && a.producer_binding_hash === s.producer_binding_hash && a.profile_digest === s.reviewer_profile.profile_digest, 'DUP_SUBJECT', row.review_id, 'assignment seal differs');
        ...
        text(a.invocation_id, 'invocation_id');
        oneOf(a.execution_kind, SEMANTIC_EXECUTIONS, 'execution_kind');
        requireDuplicate(a.round === 1 || a.round === 2, 'DUP_STATE', 'round', 'only rounds one and conditional two exist');
        invocationIds.add(a.invocation_id);
        assignments.set(row.review_id, a);
    }
    const contexts = new Set(), manualEvidence = new Map();
    for (const row of ledger.results) {
        ...
        requireDuplicate(!['agent', 'hybrid'].includes(model.manifest.mode) || ['native-dispatch', 'fixture-simulated'].includes(kind), 'DUP_ISOLATION', row.review_id, 'manual/static evidence cannot satisfy agent freshness');
        const evidence = referenced(model, row.execution_evidence_ref), record = kind === 'static-record' ? {} : parseStrictJson(evidence.bytes);
        const context = ['native-dispatch', 'fixture-simulated'].includes(kind) ? String(record.receipt.context_id) : a.invocation_id;
        ...
        if (kind === 'manual-separate-pass') {
            ...
            requireDuplicate(previous.every((r) => r.reviewer_actor !== record.reviewer_actor && r.reviewer_pass_id !== record.reviewer_pass_id
                && r.producer_actor === record.producer_actor && r.producer_pass_id === record.producer_pass_id), 'DUP_ISOLATION', row.review_id, 'conditional reviewer must be distinct from first reviewer and same producer');
            previous.push(record);
            manualEvidence.set(row.proposal_id, previous);
        }
        if (kind === 'static-record')
            requireDuplicate(existsSync(join(model.runDir, 'README.md'))
    ...
    for (const [id, s] of subjects) {
        ...
        for (const [priorId, prior] of subjects) {
            ...
            if (s.proposal.treatment === 'new-successor' && first && results.get(first.review_id)?.verdict === 'cannot-determine')
                requireDuplicate(newDuplicateBasis(s.comparison_basis, prior.comparison_basis), 'DUP_STATE', id, 'a new identity or missing predecessor link cannot erase indeterminacy over the same frozen basis');
        }
        if (s.predecessor_proposal_id) {
            ...
            if (first && results.get(first.review_id)?.verdict === 'cannot-determine' && s.proposal.treatment === 'new-successor')
                requireDuplicate(newDuplicateBasis(s.comparison_basis, prior.comparison_basis), 'DUP_STATE', id, 'post-indeterminacy absorption needs new frozen basis/context, not another declaration');
        }
    }
    ...
    for (const row of ledger.decisions) {
        ...
        requireDuplicate(row.verdict === quorum.verdict && row.reviewed_outcome === (quorum.verdict === 'upheld' ? s.proposal.outcome : 'none'), 'DUP_REVIEW', row.decision_id, 'decision aggregation/outcome differs');
    ...
    for (const row of ledger.effects) {
        ...
        oneOf(e.effect, ['canonicalized', 'kept-separate', 'not-admitted'], 'effect');
        oneOf(e.reason, DUPLICATE_REASONS, 'reason');
        strings(e.semantic_ids, 'semantic_ids');
        const attempts = parseSemanticLedger(readMaterialFile(model.runDir, SEMANTIC_PATH).toString('utf8')).subjects
            .filter((r) => r.owner_stage === 'S4').map((r) => parseSemanticJson(readMaterialFile(model.runDir, r.subject_path)))
        ...
        requireDuplicate(e.decision_id === (decision?.decision_id || null) && row.decision_id === (e.decision_id || 'none')
            && row.effect === e.effect && row.lineage_id === (e.lineage_id || 'none') && row.successor_id === (e.successor_id || 'none'), 'DUP_ACCOUNTING', row.effect_id, 'effect/table/decision join differs');
        if (!decision)
            requireDuplicate(e.effect === 'not-admitted' && e.reason === 'withdrawn-reservation'
        ...
        if (e.reason === 'reviewed-nonduplicate')
            requireDuplicate(decision?.verdict === 'upheld'
        ...
        if (e.reason === 'refuted-proposal')
            requireDuplicate(decision?.verdict === 'refuted', 'DUP_STATE', e.effect_id, 'refutation reason requires refuted DDR');
        if (e.reason === 'unresolved-origin')
            requireDuplicate(s.proposal.origin_assessment.corroboration === 'CANNOT_DETERMINE', 'DUP_STATE', e.effect_id, 'unknown origin reason requires retained unknown assessment');
        if (e.reason === 'unresolved-equivalence')
            requireDuplicate(decision?.verdict === 'cannot-determine'
                || s.proposal.outcome === 'CANNOT_DETERMINE' || s.proposal.unresolved_findings.length > 0
                || ledger.results.some((r) => r.proposal_id === s.proposal_id && results.get(r.review_id).unresolved_findings.length > 0), 'DUP_STATE', e.effect_id, 'unresolved reason requires retained uncertainty');
        if (e.reason === 'successor-not-preserved')
            requireDuplicate(attempts.length > 0
                && attempts.some((sem) => parseSemanticLedger(readMaterialFile(model.runDir, SEMANTIC_PATH).toString('utf8')).resolutions
                    .some((r) => r.semantic_id === sem.semantic_id && r.outcome === 'not-admitted')), 'DUP_STATE', e.effect_id, 'failed successor reason requires retained non-admitted SEM');
        if (e.effect === 'canonicalized') {
            ...
            requireDuplicate(first && results.get(first.review_id)?.verdict === 'upheld'
                && duplicateAdmissionProblems(s, quorums.get(s.proposal_id).verdict).length === 0 && e.reason === 'reviewed-duplicate', 'DUP_STATE', e.effect_id, 'conjunctive duplicate admission failed');
            ...
            requireDuplicate(s.reservation && e.lineage_id === s.reservation.lineage_id && e.successor_id === s.reservation.successor_id, 'DUP_ACCOUNTING', e.effect_id, 'canonical reservation differs');
            ...
            requireDuplicate(sem.owner_stage === 'S4' && e.semantic_ids.includes(sem.semantic_id) && row.semantic_id === sem.semantic_id, 'DUP_REVIEW', e.effect_id, 'admitted S4 SEM/L2S required');
            ...
            const reviewIds = parseSemanticJson(admission.review_ids);
            ...
            for (const id of reviewIds) {
                ...
                const value = parseSemanticJson(readMaterialFile(model.runDir, result.result_path));
                ...
                if (model.manifest.mode === 'manual') {
                    ...
                    requireDuplicate(object(evidence) && evidence.producer_pass_id !== producers.get(s.proposal_id).context_id
                        ...
                        && prior.every((p) => evidence.reviewer_pass_id !== p.reviewer_pass_id && evidence.producer_pass_id !== p.reviewer_pass_id), 'DUP_ISOLATION', id, 'successor producer/L2S cannot reuse duplicate producer or L3 passes');
                }
            }
            ...
            requireDuplicate(map.length === 1 && materialHash(semanticJson(map[0].cells.map(decode))) === e.merge_row_digest, 'DUP_ACCOUNTING', e.effect_id, 'exact reviewed map absent');
            requireDuplicate(map[0].values.corroboration === s.proposal.origin_assessment.corroboration, 'DUP_STATE', e.effect_id, 'canonical origin assessment differs from reviewed binary value');
            ...
            requireDuplicate(e.lineage_id === null && e.successor_id === null && e.merge_row_digest === null && row.semantic_id === 'none', 'DUP_ACCOUNTING', e.effect_id, 'noncanonical effects cannot write identities');
            if (e.effect === 'kept-separate')
                requireDuplicate(decision && (s.proposal.treatment === 'keep-separate'
                    || s.proposal.origin_assessment.corroboration === 'CANNOT_DETERMINE') && ['reviewed-nonduplicate', 'unresolved-equivalence', 'unresolved-origin'].includes(e.reason), 'DUP_STATE', e.effect_id, 'invalid kept-separate effect');
            else
                requireDuplicate(['unresolved-equivalence', 'unresolved-origin', 'refuted-proposal', 'successor-not-preserved', 'withdrawn-reservation', 'changed-prerequisite'].includes(e.reason), 'DUP_STATE', e.effect_id, 'invalid non-admission reason');
        }
        effects.set(row.proposal_id, e);
    }
    for (const row of model.merges)
        requireDuplicate([...effects.values()].filter((e) => e.effect === 'canonicalized'
            && e.successor_id === row.values.canonical && e.merge_row_digest === materialHash(semanticJson(row.cells.map(decode)))).length === 1, 'DUP_ACCOUNTING', row.values.canonical, 'merge row requires one exact canonical duplicate effect');
    if ([...effects.values()].some((e) => e.effect === 'canonicalized')) {
        ...
        requireDuplicate(!checked.checks.some((c) => c.status === 'FAIL'), 'DUP_ACCOUNTING', 'canonical lineage', checked.checks.filter((c) => c.status === 'FAIL').map((c) => c.message).join('; '));
    ...
    for (const [index, d] of discoverySequence.entries())
        for (const sweep of d.sweep_refs) {
    ...
    if (latestDiscovery && duplicatePairs(latestDiscovery.catalogue.map((c) => c.claim_id)).some(({ a, b }) => !latestDiscovery.windows.some((w) => w.member_ids.includes(a) && w.member_ids.includes(b))))
    ...
    for (const row of parseLineage(model).rows.filter((r) => r.values.ownerStage === 'S4' && ['merge', 'duplicate'].includes(r.values.type))) {
        requireDuplicate([...effects.values()].filter((e) => e.effect === 'canonicalized' && e.lineage_id === row.values.lineageId).length === 1, 'DUP_ACCOUNTING', row.values.lineageId, 'S4 absorption lacks one composed DUP effect');
    }
    ...
    if (closed !== null || duplicateStage(model) > 4) {
        ...
        equal(final.catalogue.map((c) => c.claim_id), model.claims.map((r) => r.values.claimId).filter((id) => lineageCurrentClaimIds(model).has(id)), 'DUP_ACCOUNTING', 'C1 final current inventory');
    }
    if (reachedState(model, 'ASSEMBLED')) {
        ...
        equal(tables[0].rows.map((r) => r.cells.map(decode)), duplicateSummaryRows(model).map((r) => DUPLICATE_SUMMARY_HEADERS.map((h) => r[h])), 'DUP_ACCOUNTING', 'precis.md section 17 historical duplicate finding union');
    }
...
export function duplicateRequiresWritePlan(model, path) {
...
export function validateDuplicatePlan(plan) {
    ...
    requireDuplicate(plan.stage === 'S4' && plan.key === `duplicate:${plan.proposal_id}:${plan.subject_digest}:${plan.operation}:${plan.record_id}`, 'DUP_STATE', 'plan', 'exact Core plan identity required');
    hash(plan.subject_digest, 'subject_digest');
    ...
    requireDuplicate(['S4', 'C1'].includes(expectedId) ? plan.record_id === expectedId
        : expectedId === 'VER' ? /^VER-\d+$/u.test(plan.record_id) : semanticId(plan.record_id, expectedId), 'DUP_REFERENCE', 'record_id', 'operation record namespace differs');
    const allowed = {
    ...
    for (const w of plan.writes) {
        ...
        hash(w.before_hash, 'before_hash');
        hash(w.after_hash, 'after_hash');
        ...
        if (w.path.startsWith('verification/'))
            requireDuplicate(w.before_hash === materialHash(Buffer.alloc(0)), 'DUP_STATE', w.path, 'immutable companions cannot be overwritten');
    }
    let previous = '';
    for (const p of plan.prerequisite_hashes) {
        keys(p, ['path', 'digest'], 'prerequisite');
        hash(p.digest, 'digest');
        requireDuplicate(Buffer.compare(Buffer.from(previous), Buffer.from(p.path)) < 0, 'DUP_REFERENCE', p.path, 'sorted unique prerequisites required');
        previous = p.path;
    }
    for (const b of plan.acceptance_bindings) {
        keys(b, ['call_id', 'context_id', 'raw_return_hash', 'role'], 'acceptance_binding');
        text(b.call_id, 'call_id');
        text(b.context_id, 'context_id');
        hash(b.raw_return_hash, 'raw_return_hash');
        text(b.role, 'role');
    }
}
export function planDuplicateWrite(options) {
    ...
    const controlFiles = ['control/runtime', 'control/worker-bundles', 'control/worker-returns'].flatMap((p) => existsSync(join(model.runDir, p)) ? walkFiles(join(model.runDir, p)).map((path) => relative(model.runDir, path).replaceAll('\\', '/')) : []);
    ...
    const plan = { key: `duplicate:${proposal_id}:${subject_digest}:${operation}:${record_id}`,
    ...
    for (const write of writes) {
        ...
        if (write.path.startsWith('ledgers/') && write.path !== DUPLICATE_PATH) {
            ...
            for (const table of parseTables(before.toString('utf8'))) {
                ...
                requireDuplicate(same && semanticJson(same.rows.slice(0, table.rows.length).map((r) => r.cells)) === semanticJson(table.rows.map((r) => r.cells)), 'DUP_STATE', write.path, 'composed admission cannot rewrite prior canonical rows');
            }
        }
    }
    ...
    for (const key of Object.keys(DUPLICATE_HEADERS)) {
        ...
        if (added.length)
            requireDuplicate(added[0][DUPLICATE_HEADERS[key][0]] === record_id
                && (key === 'discoveries' || added[0].proposal_id === proposal_id), 'DUP_STATE', key, 'operation targets another record');
    }
    if (operation === 'initialize')
        requireDuplicate(!existsSync(join(model.runDir, DUPLICATE_PATH))
    ...
    if (operation === 'record-discovery') {
        ...
        if (['agent', 'hybrid'].includes(model.manifest.mode)) {
            for (const window of d.windows) {
                const tuple = parseDuplicateJson(referenced(proposedModel, window.execution_evidence_ref).bytes);
    ...
    if (!['initialize', 'record-discovery', 'seal'].includes(operation)) {
        ...
        const s = parseDuplicateJson(readMaterialFile(proposedModel.runDir, row.subject_path));
        if (operation === 'reserve-subject' || operation === 'admit')
            validateDuplicateSubject(s, model, true);
        if (operation === 'reserve-subject') {
            ...
            if (s.reservation) {
                ...
                requireDuplicate(write.path === `verification/harness/semantic-process/${expected.lineage_id}.json`
                    ...
                    && row.predecessors === expected.predecessor_ids.join(', ') && row.successors === expected.successor_id, 'DUP_REFERENCE', 'reservation', 'exact reviewed group/type/new successor required');
                text(row.basis, 'basis');
                text(row.established_by, 'established_by');
            }
        }
        if (operation === 'assign-review') {
            const a = parseDuplicateJson(readMaterialFile(proposedModel.runDir, duplicatePath('assignments', record_id)));
            if (a.round === 2) {
                ...
                requireDuplicate(result && parseDuplicateJson(readMaterialFile(model.runDir, result.result_path)).verdict === 'cannot-determine', 'DUP_STATE', 'round two', 'round-one unknown result must precede assignment');
            }
        }
        if (operation === 'record-effect' || operation === 'admit') {
            const effect = referenced(proposedModel, after.effects.at(-1).record_ref);
            const e = parseDuplicateJson(effect.bytes);
        ...
        if (['agent', 'hybrid'].includes(model.manifest.mode) || acceptance_bindings.length) {
            const producer = referenced(proposedModel, row.producer_receipt_ref), tuple = parseDuplicateJson(producer.bytes);
            requireDuplicate(acceptance_bindings.some((b) => b.call_id === tuple.call_id && b.context_id === tuple.context_id
            ...
            if (operation === 'record-review') {
                ...
                const evidence = parseStrictJson(referenced(proposedModel, result.execution_evidence_ref).bytes);
                requireDuplicate(acceptance_bindings.some((b) => b.call_id === a.invocation_id && b.role === 'verifier-l3'
                    && b.context_id === evidence.receipt?.context_id && b.raw_return_hash === result.result_digest), 'DUP_ISOLATION', 'accepted reviewer', 'exact accepted L3 call/context/raw return required');
            }
            if (operation === 'admit') {
                for (const row of after.assignments.filter((r) => r.proposal_id === proposal_id)) {
                    const a = parseDuplicateJson(readMaterialFile(proposedModel.runDir, row.assignment_path));
                    requireDuplicate(acceptance_bindings.some((b) => b.call_id === a.invocation_id && b.role === 'verifier-l3'), 'DUP_ISOLATION', row.review_id, 'admission requires the actual accepted L3 result');
                }
                const e = parseDuplicateJson(referenced(proposedModel, after.effects.at(-1).record_ref).bytes);
                ...
                validateSemanticAcceptedBindings(proposedModel, sem.semantic_id, 'admit', producer, acceptance_bindings.filter((b) => ['verifier-l2s', 'verifier-l2f'].includes(b.role)));
            }
        }
    }
    ...
    if (operation === 'seal')
        requireDuplicate(!report.pending.length && subject_digest === materialHash(readMaterialFile(model.runDir, DUPLICATE_PATH))
            && duplicateClosureHash(proposedModel) === subject_digest, 'DUP_WINDOW', 'seal', 'complete additional C1 seal required');
    return plan;
}
export function duplicateAdmissionSubplans(model, proposedModel, plan) {
    if (plan.operation !== 'admit')
        return null;
    ...
    validateDuplicateAdmissionSubplans(proposedModel, plan, result);
    return result;
}
export function validateDuplicateAdmissionSubplans(model, plan, subplans) {
    if (plan.operation !== 'admit') {
        requireDuplicate(subplans === null, 'DUP_STATE', 'subplans', 'only admission composes successor plans');
        return;
    }
    ...
    requireDuplicate(subplans.semantic_plan.semantic_id === effect.semantic_id && subplans.material_plan.inventory_hash === context.inventoryHash, 'DUP_SUBJECT', 'subplans', 'successor/capture identity differs');
    for (const subplan of [subplans.semantic_plan, subplans.material_plan])
        for (const w of subplan.writes)
...
export function validateCompletedDuplicatePlan(model, plan) {
    validateDuplicatePlan(plan);
    ...
    for (const w of plan.writes) {
        ...
        if (w.path === DUPLICATE_PATH) {
            ...
            for (const key of Object.keys(DUPLICATE_HEADERS))
                equal(historical[key], ledger[key].slice(0, historical[key].length), 'DUP_STATE', key);
        }
            ...
            requireDuplicate(w.path.startsWith('verification/') ? current.equals(after) : current.subarray(0, after.length).equals(after), 'DUP_STATE', w.path, 'committed immutable file/prefix changed');
    }
}
const closedSchema = (properties) => ({ type: 'object', properties, required: Object.keys(properties), additionalProperties: false });
const stringSchema = (pattern) => ({ type: 'string', minLength: 1, ...(pattern ? { pattern } : {}) });
const enumSchema = (values) => ({ type: 'string', enum: [...values] });
const arraySchema = (items, minItems = 0) => ({ type: 'array', items, minItems });
const nullableSchema = (schema) => ({ anyOf: [schema, { type: 'null' }] });
export function duplicateReturnJsonSchema(task, version) {
    requireDuplicate(hasRunCapability(version, 'duplicate-overlap-review'), 'DUP_COMPATIBILITY', 'schema', 'capability required');
    oneOf(task, Object.keys(DUPLICATE_TASKS), 'task');
    ...
    if (task === 'discovery')
        return closedSchema({ candidates: arraySchema(candidate),
    ...
    if (task === 'contradiction-discovery')
        return closedSchema({
    ...
    if (task === 'refutation')
        return closedSchema({ format: enumSchema([DUPLICATE_RESULT_FORMAT]), subject_digest: sha, verdict: enumSchema(DUPLICATE_VERDICTS),
...
function schemaShape(value, schema, path = '$') {
    requireDuplicate(object(schema), 'DUP_FORMAT', path, 'schema object required');
    if (Array.isArray(schema.anyOf)) {
        requireDuplicate(schema.anyOf.some((s) => { try {
            schemaShape(value, s, path);
            return true;
        }
        catch {
            return false;
        } }), 'DUP_FORMAT', path, 'no legal schema variant');
        return;
    }
    if (schema.type === 'null') {
        requireDuplicate(value === null, 'DUP_FORMAT', path, 'null required');
        return;
    }
    if (schema.type === 'object') {
        ...
        for (const [key, s] of Object.entries(schema.properties))
            schemaShape(value[key], s, `${path}/${key}`);
    }
        ...
        requireDuplicate(integer(value) && value >= Number(schema.minimum), 'DUP_FORMAT', path, 'safe ordinal required');
    else {
        text(value, path);
        if (Array.isArray(schema.enum))
            requireDuplicate(schema.enum.includes(value), 'DUP_ENUM', path, 'unknown enum');
        if (typeof schema.pattern === 'string')
            requireDuplicate(new RegExp(schema.pattern, 'u').test(value), 'DUP_REFERENCE', path, 'reference grammar differs');
    }
}
export function duplicateOutputContract(task) {
...
export function isDuplicateOutputContract(v) { return object(v) && v.contract_format === DUPLICATE_CONTRACT_FORMAT; }
export function validateDuplicateOutputContract(v) {
    keys(v, ['contract_format', 'capability', 'task', 'role', 'shape'], 'duplicate output contract');
    oneOf(v.task, Object.keys(DUPLICATE_TASKS), 'task');
    equal(v, duplicateOutputContract(v.task), 'DUP_FORMAT', 'Core contract descriptor');
    return v.task;
}
export function validateDuplicateReturn(task, version, value, context) {
    try {
        duplicateJson(value);
        schemaShape(value, duplicateReturnJsonSchema(task, version));
        ...
        if (context) {
            ...
            if (task === 'refutation' && context.subject)
                validateDuplicateSubject(context.subject, context.model);
            else if (task === 'comparison' && context.basis) {
                const proposal = returned.proposal;
                if (context.candidate_ref)
                    equal(proposal.candidate_ref, context.candidate_ref, 'DUP_SUBJECT', 'assigned candidate');
        ...
        if (task === 'refutation')
            validateDuplicateResult(value, context?.subject);
        else if (task === 'comparison') {
            validateDuplicateProposal(returned.proposal, context?.basis, context?.basis ? duplicateComparisonFields(context.basis) : undefined);
            rationale(returned.rationale);
        }
        else if (task === 'discovery') {
            for (const c of returned.candidates) {
                strings(c.member_ids, 'members', true);
                if (context?.member_ids)
                    subset(c.member_ids, context.member_ids, 'current discovery members', true);
            }
            for (const f of returned.unresolved_findings)
                requestedContext(f.requested_context);
            rationale(returned.rationale);
        }
        else {
            requireDuplicate(returned.verdict === 'refuted' || returned.flagged_pairs.length === 0, 'DUP_REVIEW', 'L5', 'flagged pairs only on refuted');
            for (const pair of returned.flagged_pairs)
                requireDuplicate(pair.a !== pair.b
        ...
        return { result: 'PASS', errors: [], canonicalValue: value,
...
export function duplicateTaskForRole(version, role, stage, taskLine) {
    if (!hasRunCapability(version, 'duplicate-overlap-review') || stage !== 'S4')
        return null;
    if (role === 'verifier-l3')
        return taskLine === DUPLICATE_TASKS.refutation ? 'refutation' : null;
    if (role === 'verifier-l5')
        return 'contradiction-discovery';
    if (role !== 'merge-judge')
        return null;
...
export function validateDuplicateRoleDelivery(version, role, stage, taskLine, paths) {
    if (!hasRunCapability(version, 'duplicate-overlap-review'))
        return;
    if (paths.some((path) => /^verification\/harness\/duplicate-subjects\/DUP-\d+\.json$/u.test(path)))
        requireDuplicate(role === 'verifier-l3' && stage === 'S4' && taskLine === DUPLICATE_TASKS.refutation, 'DUP_ISOLATION', 'sealed comparison delivery', 'duplicate subject requires its exact bounded L3 task and role');
}
export function duplicateProducerView(model, task, selection) {
    assertDuplicateWindow(model);
    if (task === 'comparison') {
        keys(selection, ['candidate_ref'], 'comparison selection');
        text(selection.candidate_ref, 'candidate_ref');
    ...
    if (task === 'discovery')
        return { bytes: Buffer.from(semanticJson(selected)), context: { model, member_ids: selection.member_ids } };
    return { bytes: duplicateSweepView(model, selected),
        context: { model, member_ids: selection.member_ids } };
}
export function duplicateProducerPaths(callId) {
...
export function validateDuplicateProducerDelivery(model, task, callId, taskLine, attachments) {
    const paths = duplicateProducerPaths(callId);
    const selection = parseDuplicateJson(readMaterialFile(model.runDir, paths.selection));
...
export function validateDuplicateReviewDispatch(model, callId, taskLine, producerContext, attachments, retained = false) {
    if (!retained)
        assertDuplicateWindow(model);
    ...
    const subject = parseDuplicateJson(paths[0].bytes);
    validateDuplicateSubject(subject, model);
    validateDuplicateAttachmentDelivery(subject, taskLine, attachments);
    const ledger = parseDuplicateLedger(readMaterialFile(model.runDir, DUPLICATE_PATH).toString('utf8'));
    const assignment = ledger.assignments.map((r) => parseDuplicateJson(readMaterialFile(model.runDir, r.assignment_path)))
    ...
    const producer = parseDuplicateJson(referenced(model, row.producer_receipt_ref).bytes);
...
export function duplicateSuccessorSubject(model, lineageId, retained = false) {
    if (!retained)
        assertDuplicateWindow(model);
    validateDuplicateRun(model);
    const ledger = parseDuplicateLedger(readMaterialFile(model.runDir, DUPLICATE_PATH).toString('utf8'));
    const subjects = ledger.proposals.map((r) => parseDuplicateJson(readMaterialFile(model.runDir, r.subject_path)))
...
export function validateDuplicateBundleDelivery(model, task, callId, taskLine, producerContext, attachments) {
    if (task === 'refutation') {
        validateDuplicateReviewDispatch(model, callId, taskLine, producerContext, attachments, true);
        return;
    }
    ...
    if (task === 'comparison')
        for (const row of ledger.proposals) {
            const tuple = parseDuplicateJson(referenced(model, row.producer_receipt_ref).bytes);
            if (tuple.call_id !== callId)
                continue;
            const s = parseDuplicateJson(readMaterialFile(model.runDir, row.subject_path));
            validateDuplicateSubject(s, model);
    ...
    for (const row of ledger.discoveries) {
        ...
        if (task === 'discovery')
            for (const w of d.windows) {
                const tuple = parseDuplicateJson(referenced(model, w.execution_evidence_ref).bytes);
                if (tuple.call_id === callId)
                    expected = Buffer.from(semanticJson(d.catalogue.filter((c) => w.member_ids.includes(c.claim_id))));
            }
        if (task === 'contradiction-discovery')
            for (const sweep of d.sweep_refs) {
    ...
    if (!expected) {
        validateDuplicateProducerDelivery(model, task, callId, taskLine, attachments);
        return;
    }
    requireDuplicate(taskLine === DUPLICATE_TASKS[task] && attachments.length === 1 && attachments[0].path === paths.view
        && attachments[0].bytes.equals(expected), 'DUP_ISOLATION', callId, 'retained exact delivery required');
}
export function duplicateProposalState(ledger, proposalId) {
    ...
    if (effect) {
        oneOf(effect.effect, ['canonicalized', 'kept-separate', 'not-admitted'], 'effect');
        return effect.effect;
    }
    if (ledger.decisions.some((r) => r.proposal_id === proposalId))
        return 'reviewed';
    return ledger.assignments.some((r) => r.proposal_id === proposalId) ? 'review-pending' : 'proposed';
}
export const DUPLICATE_SUMMARY_HEADERS = ['proposal_id', 'subject_digest', 'member_ids', 'finding_ref', 'review_verdict', 'effect'];
function duplicateSummaryRows(model) {
    const ledger = parseDuplicateLedger(readMaterialFile(model.runDir, DUPLICATE_PATH).toString('utf8')), rows = [];
    for (const row of ledger.proposals) {
        const s = parseDuplicateJson(readMaterialFile(model.runDir, row.subject_path));
        ...
        for (const r of ledger.results.filter((r) => r.proposal_id === row.proposal_id)) {
            const result = parseDuplicateJson(readMaterialFile(model.runDir, r.result_path));
            refs.push(...result.unresolved_findings.map((f) => `${r.review_id}:${f.finding_id}`));
            if (result.verdict === 'refuted')
                refs.push(`${r.review_id}:refuted`);
            refs.push(...result.contradiction_pairs.map((p) => `${r.review_id}:flagged:${p.a}/${p.b}`));
        }
        if (effect) {
            const e = parseDuplicateJson(referenced(model, effect.record_ref).bytes);
            if (e.reason === 'successor-not-preserved')
                refs.push(`${e.effect_id}:successor-not-preserved`);
        }
        for (const finding_ref of refs)
            rows.push({ proposal_id: row.proposal_id, subject_digest: row.subject_digest,
                member_ids: semanticJson(s.proposal.member_ids), finding_ref, review_verdict: decision?.verdict || 'none', effect: effect?.effect || 'none' });
    }
    return rows;
}
export function duplicateUnresolvedSummary(model) {
    if (!existsSync(join(model.runDir, DUPLICATE_PATH)))
        return '';
    validateDuplicateRun(model);
