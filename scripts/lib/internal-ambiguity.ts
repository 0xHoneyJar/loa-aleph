...
export function loadPinnedCoreAuthority(options: {
  ...
  let lock: BundleLock;
  try {
    lock = JSON.parse(raw.toString('utf8')) as BundleLock;
  } catch {
  ...
  for (const record of lock.files) {
    ...
    if (!existsSync(path)) throw new Error(`retained Core file is absent: ${record.path}`);
...
export interface StructuredVerifierRecord {
  target: string;
  lens: string;
  stage: string;
  shown: string;
...
export interface RetainedVerifierFile {
  path: string;
  bytes: Buffer;
}

export interface MaterialImpactAuthorityBasis {
  material_subject: MaterialImpactSubject;
  material_subject_digest: string;
  material_subject_ref: string;
  material_review_id: string;
  material_review_ref: string;
  material_review: StructuredVerifierRecord;
}

export interface RestrictionTuple {
  affected_id: string;
  operation_kind: OperationKind;
  requirement_ref: string;
}
...
export function parseStructuredVerifierRecord(
  bytes: Buffer,
  path = 'verifier record',
): StructuredVerifierRecord {
  ...
  for (const row of tables[0].rows) {
    ...
    if (!(STRUCTURED_VERIFIER_FIELDS as readonly string[]).includes(field)) continue;
  ...
  return {
    ...
    verdict: verdict as StructuredVerifierRecord['verdict'],
    consequence: values.get('consequence')![0],
  };
}

export function validateMaterialImpactAuthorityBasis(options: {
  material_subject_bytes: Buffer;
  verifier_files: readonly RetainedVerifierFile[];
  authority_subject: ProceduralAuthoritySubject;
}): MaterialImpactAuthorityBasis {
  let material: MaterialImpactSubject;
  try {
    material = JSON.parse(options.material_subject_bytes.toString('utf8')) as MaterialImpactSubject;
  } catch {
...
export function restrictionOverlay(scope: OperativeScope): RestrictionTuple[] {
...
function restrictionTupleKey(value: RestrictionTuple): string {
  return `${value.affected_id}\0${value.operation_kind}\0${value.requirement_ref}`;
}

export function retainedRestrictionOverlays(model: RunModel): RestrictionTuple[] {
  const retained = new Map<string, RestrictionTuple>();
  const ambiguity = parseInternalAmbiguities(model);
  for (const row of ambiguity.t5_3Rows) {
    if (row.values.action !== 'restrict-downstream-use') continue;
    const provenance = row.values.closureProvenance.match(
      /^request:(GATE-S4-AMB-\d{4,}-A[1-9]\d*-Q[1-9]\d*)@(sha256:[a-f0-9]{64});response:(RESP-S4-AMB-\d{4,}-A[1-9]\d*-Q[1-9]\d*)@(sha256:[a-f0-9]{64})$/u,
    );
    if (!provenance) {
  ...
  return [...retained.values()].sort((left, right) => (
    Buffer.from(restrictionTupleKey(left), 'utf8')
      .compare(Buffer.from(restrictionTupleKey(right), 'utf8'))
  ));
}

export function assertDownstreamOperationsAllowed(
  restrictions: readonly RestrictionTuple[],
  operations: readonly RestrictionTuple[],
): void {
  const prohibited = new Set(restrictions.map(restrictionTupleKey));
  const seen = new Set<string>();
  for (const operation of operations) {
    if (!/^(?:PKT|CC|REL)-\d{4,}$/u.test(operation.affected_id)
      || !(OPERATION_KINDS as readonly string[]).includes(operation.operation_kind)) {
    ...
    if (seen.has(key)) throw new Error('downstream operation tuple is duplicated');
    seen.add(key);
    if (prohibited.has(key)) {
      throw new Error(
        `downstream operation is prohibited by retained procedural restriction: `
