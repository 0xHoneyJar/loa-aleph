...
import { findTables, normalizeHeader, parseBulletFields } from './markdown.ts';
...
export const RELATION_FAMILY_TYPES: Readonly<Record<
  typeof RELATION_FAMILIES[number],
  readonly typeof RELATION_TYPES[number][]
>> = {
...
export interface RelationRow extends MarkdownTableRow {
...
export interface RelationModel {
  ...
  format: string;
  relationTables: MarkdownTable[];
  canonicalTables: MarkdownTable[];
  table: MarkdownTable | null;
  rows: RelationRow[];
}

export interface ParsedPacketBasis {
  ids: string[];
  clean: boolean;
}

export type RelationWritePhase =
  | 'before-s4-closure'
  | 's4-closure'
  | 'after-s4-closure';

export function parsePacketBasis(value: string): ParsedPacketBasis {
...
export function parseRelations(model: RunModel): RelationModel {
  ...
  const relationTables = document.tables.filter(
    (table) => normalizeHeader(table.header[0]) === 'relation id',
  );
  const table = canonicalTables[0] || null;
  const rows = (table?.rows || []).map((row): RelationRow => ({
    ...row,
    values: {
...
export function relationReviewSubjectJson(values: RelationValues): string {
...
export function relationReviewSubjectDigest(values: RelationValues): string {
  return `sha256:${createHash('sha256')
    .update(Buffer.from(relationReviewSubjectJson(values), 'utf8'))
    .digest('hex')}`;
}

export function requireRelationWriteWindow(phase: RelationWritePhase): void {
  if (phase !== 's4-closure') {
