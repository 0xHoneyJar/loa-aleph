...
import { hasRunLogEvent, mdLineSpan, reachedState, runLogEvents, sourceFilePath } from './check-helpers.ts';
import { closurePhasesFromText, parseStructuredVerifierRecord } from './internal-ambiguity.ts';
import { envelopeSection, parseTables } from './markdown.ts';
...
export const REPRESENTATION_STATES = ['available', 'degraded', 'unsupported', 'indeterminate'] as const;
...
export type MaterialRow = Record<string, string>;
export type InventoryTable = Exclude<keyof typeof MATERIAL_HEADERS, 'uses'>;
export type MaterialInventory = Record<InventoryTable, MaterialRow[]>;
export interface MaterialRequirement { object_id: string; feature: string; binding_ids: string[] }
...
export interface MaterialCarrier { bytes: Buffer; encoding: 'utf8' | 'opaque'; representation_id: string }
export interface RepresentationContext {
  inventory: MaterialInventory;
  inventoryHash: string;
  carriers: Map<string, MaterialCarrier>;
  uses: MaterialRow[];
}
export class RepresentationError extends Error {
  constructor(token: string, subject: string, field: string, reason: string) {
    super(`${token} ${subject} field ${field}: ${reason}`);
  }
}
function requireMaterial(condition: unknown, token: string, subject: string, field: string, reason: string): asserts condition {
  if (!condition) throw new RepresentationError(token, subject, field, reason);
}
export function materialHash(bytes: Buffer | string): string {
  return `sha256:${createHash('sha256').update(bytes).digest('hex')}`;
}
export function materialFragmentsHash(fragments: readonly Buffer[]): string {
...
function json(value: string, id: string, field: string): unknown {
  let parsed: unknown;
  try { parsed = parseStrictJson(value); } catch {
...
function list(value: string, id: string, field: string, sorted = false): string[] {
  ...
  if (sorted) requireMaterial(JSON.stringify([...parsed].sort()) === value, 'FORMAT', id, field, 'set must be ASCII sorted');
  return parsed as string[];
}
function idFor(value: string, family: string): boolean {
  return new RegExp(`^${family}-(?=\\d*[1-9])\\d{4,}$`, 'u').test(value);
}
function integer(value: string, id: string, field: string, positive = false): number {
...
function state(row: MaterialRow, key = 'state'): void {
  requireMaterial((REPRESENTATION_STATES as readonly string[]).includes(row[key]), 'STATE', rowId(row), key, 'unknown state');
  requireMaterial(row[key] === 'available' ? row.reason === 'none' : Boolean(row.reason?.trim()) && row.reason !== 'none',
    'STATE', rowId(row), 'reason', 'state/reason mismatch');
}
function rowId(row: MaterialRow): string {
  for (const key of ['use_id', 'asset_id', 'binding_id', 'object_id', 'association_id']) if (key in row) return row[key];
  return ('origin_kind' in row ? row.representation_id : row.provenance_id) || REPRESENTATION_PATH;
}
function find(rows: MaterialRow[], id: string, token = 'IDENTITY'): MaterialRow {
  const row = rows.find((candidate) => rowId(candidate) === id);
  requireMaterial(row, token, id, 'reference', 'missing declared row');
  return row;
}
function sameRep(left: MaterialRow, right: MaterialRow, field: string): void {
...
export function materialTableMarkdown(header: readonly string[], rows: readonly MaterialRow[]): string {
  return `| ${header.join(' | ')} |\n| ${header.map(() => '---').join(' | ')} |\n`
    + rows.map((row) => `| ${header.map((key) => escapeCell(row[key])).join(' | ')} |\n`).join('');
}
function tableRows(text: string, name: keyof typeof MATERIAL_HEADERS, path: string): MaterialRow[] {
  ...
  requireMaterial(matching.length === 1 && JSON.stringify(matching[0].header) === JSON.stringify(header),
    'FORMAT', path, header[0], 'require one table with exact headers');
...
export function emptyMaterialInventory(): MaterialInventory {
  return { representations: [], assets: [], provenance: [], bindings: [], objects: [], associations: [] };
}
export function representationMarkdown(inventory: MaterialInventory): string {
  return `# Source Representations\n\n- source_representation_format: ${REPRESENTATION_FORMAT}\n\n`
    + (Object.keys(inventory) as InventoryTable[]).map((key) =>
      `## ${key}\n\n${materialTableMarkdown(MATERIAL_HEADERS[key], inventory[key])}`).join('\n');
}
export function representationUsesMarkdown(rows: readonly MaterialRow[]): string {
...
function marker(text: string, field: string, value: string, path: string): void {
  const candidates = text.split(/\r?\n/u).filter((line) => line.includes(`${field}:`));
  requireMaterial(candidates.length === 1 && candidates[0] === `- ${field}: ${value}`,
    'FORMAT', path, field, 'require one exact marker');
}
export function parseRepresentationInventory(text: string): MaterialInventory {
  marker(text, 'source_representation_format', REPRESENTATION_FORMAT, REPRESENTATION_PATH);
  const inventory = emptyMaterialInventory();
  for (const key of Object.keys(inventory) as InventoryTable[]) inventory[key] = tableRows(text, key, REPRESENTATION_PATH);
  requireMaterial(parseTables(text).length === 6, 'FORMAT', REPRESENTATION_PATH, 'tables', 'exactly six canonical tables required');
  return inventory;
}
export function readMaterialFile(root: string, path: string): Buffer {
  requireMaterial(path !== '' && !path.startsWith('/') && !/[\\:\0]/u.test(path)
    && path.split('/').every((part) => part && part !== '.' && part !== '..'),
  'CAPTURE_HASH', path, 'locus', 'unsafe relative path');
  let current = root;
...
function bindingBytes(context: RepresentationContext, binding: MaterialRow): Buffer {
...
function objectBindings(context: RepresentationContext, object: MaterialRow): MaterialRow[] {
  return list(object.binding_ids, rowId(object), 'binding_ids').map((id) => find(context.inventory.bindings, id, 'BINDING'));
}
function ancestors(inventory: MaterialInventory, object: MaterialRow): MaterialRow[] {
  const result = [object];
...
function coordinates(object: MaterialRow): Record<string, unknown> {
  const value = json(object.coordinates, rowId(object), 'coordinates');
  requireMaterial(COORDINATE_KEYS[object.kind] && exactKeys(value, COORDINATE_KEYS[object.kind]),
    'COORDINATE', rowId(object), 'coordinates', 'unknown kind or noncanonical coordinate shape');
  return value;
}
function coordinateIds(value: unknown, object: MaterialRow, field: string): string[] {
  requireMaterial(Array.isArray(value) && value.every((id) => typeof id === 'string') && new Set(value).size === value.length,
    'COORDINATE', rowId(object), field, 'expected ordered unique IDs');
  return value as string[];
}
function positive(value: unknown): value is number { return typeof value === 'number' && Number.isSafeInteger(value) && value > 0; }
function coordinateObjects(context: RepresentationContext): void {
  const inv = context.inventory;
  const byId = (id: string) => find(inv.objects, id, 'COORDINATE');
  for (const object of inv.objects) {
    ...
    requireMaterial(object.kind === 'source' ? object.parent_id === 'none'
      : parents.length > 1 && PARENTS[object.kind].includes(parents[1].kind),
    'COORDINATE', id, 'parent_id', 'illegal parent kind');
    if (object.kind === 'page') {
      ...
      requireMaterial(c.space === null || (exactKeys(c.space, ['unit', 'width', 'height', 'origin'])
        && c.space.unit === 'pixel' && c.space.origin === 'top-left' && positive(c.space.width) && positive(c.space.height)),
      'COORDINATE', id, 'space', 'expected supplied integer pixel space');
    }
    if (object.kind === 'region') {
      requireMaterial(c.page_id === null || typeof c.page_id === 'string', 'COORDINATE', id, 'page_id', 'invalid page');
      const page = c.page_id === null ? null : byId(c.page_id as string);
      if (page) { sameRep(object, page, 'page_id'); requireMaterial(page.kind === 'page', 'COORDINATE', id, 'page_id', 'not a page'); }
      ...
      if (c.box !== null) {
        ...
        requireMaterial(record(space) && exactKeys(c.box, ['x0', 'y0', 'x1', 'y1'])
          && Object.values(c.box).every((v) => typeof v === 'number' && Number.isSafeInteger(v) && v >= 0),
        'COORDINATE', id, 'box', 'box requires declared integer page space');
        const box = c.box as Record<string, number>;
        requireMaterial(box.x0 < box.x1 && box.y0 < box.y1 && box.x1 <= Number(space.width) && box.y1 <= Number(space.height),
          'COORDINATE', id, 'box', 'box outside supplied page space');
      }
    }
    if (object.kind === 'row' || object.kind === 'column') requireMaterial(positive(c.index), 'COORDINATE', id, 'index', 'invalid index');
    if (object.kind === 'table') {
      const rows = coordinateIds(c.row_ids, object, 'row_ids'), columns = coordinateIds(c.column_ids, object, 'column_ids');
      requireMaterial((REPRESENTATION_STATES as readonly unknown[]).includes(c.grid_state), 'STATE', id, 'grid_state', 'unknown state');
      ...
      for (const [kind, ids] of [['row', rows], ['column', columns]] as const) {
        ...
        if (c.grid_state === 'available') requireMaterial(ids.length > 0 && indices.every((v, i) => v === i + 1),
          'COORDINATE', id, `${kind}_ids`, 'available axis must be positive and contiguous');
      }
      ...
      for (const cell of inv.objects.filter((child) => child.parent_id === id && child.kind === 'cell')) {
        ...
        if (rr.length === 0 || cs.length === 0) {
          requireMaterial(rr.length === 0 && cs.length === 0 && ['indeterminate', 'unsupported'].includes(cell.state),
            'COORDINATE', rowId(cell), 'row_ids/column_ids', 'unpositioned cell must have both axes empty');
        } else {
          for (const [selected, axis] of [[rr, rows], [cs, columns]]) {
            const start = axis.indexOf(selected[0]);
            requireMaterial(start >= 0 && JSON.stringify(axis.slice(start, start + selected.length)) === JSON.stringify(selected),
              'COORDINATE', rowId(cell), 'span', 'not a contiguous declared axis subsequence');
          }
          for (const row of rr) for (const col of cs) {
            const key = `${row}/${col}`;
      ...
      requireMaterial(c.grid_state !== 'available' || occupied.size === rows.length * columns.length,
        'COORDINATE', id, 'grid_state', 'available grid has missing cells');
    }
    if (object.kind === 'header') {
      const cells = coordinateIds(c.cell_ids, object, 'cell_ids').map(byId);
      for (const cell of cells) requireMaterial(cell.kind === 'cell' && cell.parent_id === object.parent_id,
        'COORDINATE', id, 'cell_ids', 'header cell belongs to another table');
      if (cells.length) for (const b of objectBindings(context, object)) {
        const spans = cells.flatMap((cell) => objectBindings(context, cell)).filter((x) => x.carrier_id === b.carrier_id)
          .map((x) => [Number(x.start_byte), Number(x.end_byte)] as const);
    ...
    if (object.kind === 'formal' || object.kind === 'figure') {
      ...
      requireMaterial((REPRESENTATION_STATES as readonly unknown[]).includes(featureState)
      ...
      for (const [field, kind] of fields) for (const ref of coordinateIds(c[field], object, field)) {
        const target = byId(ref); sameRep(object, target, field);
        requireMaterial(target.kind === kind, 'COORDINATE', id, field, 'wrong target kind');
      }
      if (formal && c.notation === 'flattened-text') requireMaterial(c.structure_state === 'degraded' && (c.structure_ids as unknown[]).length === 0,
        'STATE', id, 'structure_state', 'flattened text cannot supply recovered structure');
      if (!formal && featureState === 'available') requireMaterial((c.values_ids as unknown[]).length > 0, 'STATE', id, 'values_ids', 'available chart values need declarations');
    }
    ...
    for (const b of objectBindings(context, object)) {
      if (fixedRegion && b.region_id !== 'none') requireMaterial(b.region_id === rowId(fixedRegion), 'COORDINATE', id, 'region_id', 'binding contradicts ancestor');
...
function intervalCovered(start: number, end: number, spans: readonly (readonly [number, number])[]): boolean {
  let cursor = start;
...
export function validateMaterialInventory(context: RepresentationContext): void {
  const inv = context.inventory;
  const families: Record<InventoryTable, string> = { representations: 'REP', assets: 'AST', provenance: 'RPR', bindings: 'BND', objects: 'OBJ', associations: 'ASC' };
  for (const key of Object.keys(inv) as InventoryTable[]) {
    ...
    for (const row of inv[key]) {
      ...
      ids.add(id);
      if (key !== 'representations') find(inv.representations, row.representation_id);
      if ('state' in row) state(row);
    }
  }
  for (const rep of inv.representations) {
    ...
    requireMaterial(['utf8-text', 'opaque'].includes(rep.extraction_surface) && (rep.extraction_surface !== 'opaque' || rep.state !== 'available'),
      'STATE', rowId(rep), 'extraction_surface', 'unsupported or falsely available surface');
    ...
    requireMaterial(rootBindings.length === 1 && rootBindings[0].carrier_id === rep.source_id
      && rootBindings[0].start_byte === '0' && Number(rootBindings[0].end_byte) === carrier.bytes.length,
    'INVENTORY', rowId(rep), 'root', 'root must bind complete capture');
  }
  const outputs = new Map<string, MaterialRow>();
  for (const p of inv.provenance) {
    ...
    requireMaterial(out.length > 0 && (p.type === 'capture' ? inputs.length === 0 : inputs.includes(rep.source_id)),
      'PROVENANCE', id, 'input_refs/output_refs', 'invalid provenance closure');
    for (const ref of [...inputs, ...out]) requireMaterial(context.carriers.get(ref)?.representation_id === p.representation_id,
      'PROVENANCE', id, 'refs', 'unknown or foreign carrier');
    ...
    if (p.type === 'capture') {
      ...
      for (const ref of out.filter((v) => v !== rep.source_id)) {
        ...
        requireMaterial(asset.role === 'upstream-capture' || (ref === p.declaration_asset_id && asset.role === 'render-log'),
          'PROVENANCE', id, 'output_refs', 'capture output must be upstream bytes or imported ID map');
      }
    } else if (p.type === 'supplied-structure') {
      requireMaterial(p.parameters_asset_id === 'none' && out.includes(p.declaration_asset_id)
        && find(inv.assets, p.declaration_asset_id, 'PROVENANCE').role === 'structure-export',
      'PROVENANCE', id, 'declaration_asset_id', 'retained structure declaration required');
    } else {
      for (const key of ['parameters_asset_id', 'declaration_asset_id']) requireMaterial(out.includes(p[key])
  ...
  for (const asset of inv.assets) {
    ...
    function visit(ref: string): void {
      ...
      seen.add(ref);
      for (const input of list(outputs.get(ref)?.input_refs || '[]', ref, 'input_refs')) visit(input);
      seen.delete(ref);
    }
    visit(rowId(asset));
  }
  for (const b of inv.bindings) {
    ...
    requireMaterial(b.exact_bytes_base64 === (bytes.length ? bytes.toString('base64') : '""') && b.fragment_hash === materialHash(bytes),
      'BINDING', id, 'exact_bytes_base64/fragment_hash', 'exact bytes disagree');
    let region: MaterialRow | null = null;
    for (const kind of ['page', 'region']) if (b[`${kind}_id`] !== 'none') {
      const target = find(inv.objects, b[`${kind}_id`], 'COORDINATE'); sameRep(b, target, `${kind}_id`);
      requireMaterial(target.kind === kind, 'COORDINATE', id, `${kind}_id`, 'wrong locator kind');
      if (kind === 'region') region = target;
    }
    requireMaterial(!region || b.page_id === 'none' || coordinates(region).page_id === b.page_id,
      'COORDINATE', id, 'page_id', 'region page mismatch');
  }
  for (const o of inv.objects) {
    ...
    for (const b of bs) sameRep(o, b, 'binding_ids');
    requireMaterial(o.content_hash === (bs.length ? materialFragmentsHash(bs.map((b) => bindingBytes(context, b))) : 'none'),
      'BINDING', id, 'content_hash', 'ordered fragments disagree');
    if (p.type === 'capture') {
    ...
    if (o.state === 'available') {
      ...
      else if (!['row', 'column'].includes(o.kind)) requireMaterial(bs.length > 0 || inv.objects.some((child) =>
        child.binding_ids !== '[]' && ancestors(inv, child).some((a) => rowId(a) === id)), 'BINDING', id, 'binding_ids', 'unbound container');
    }
    if (o.kind === 'image') requireMaterial(bs.every((b) => {
    ...
    if (o.kind === 'chart-values') requireMaterial(bs.every((b) => context.carriers.get(b.carrier_id)?.encoding === 'utf8'),
      'BINDING', id, 'binding_ids', 'chart values require supplied textual bytes');
  }
  coordinateObjects(context);
  for (const o of inv.objects) {
    const kind = o.kind === 'cell' ? 'header-for' : ['table', 'formal', 'figure'].includes(o.kind) ? 'caption-for' : null;
    if (kind) requireMaterial(inv.associations.filter((a) => a.subject_id === rowId(o) && a.kind === kind).length === 1,
      'ASSOCIATION', rowId(o), kind, 'require one explicit association including absence/candidates');
  }
  for (const a of inv.associations) {
    ...
    sameRep(a, subject, 'subject_id'); sameRep(a, p, 'provenance_id');
    ...
    requireMaterial((a.state !== 'available' || targets.length > 0) && (a.state !== 'unsupported' || targets.length === 0),
      'ASSOCIATION', id, 'target_ids', 'association state/target mismatch');
    for (const ref of targets) {
      const target = find(inv.objects, ref, 'ASSOCIATION'); sameRep(a, target, 'target_ids');
      requireMaterial(a.kind === 'header-for' ? target.kind === 'header' && target.parent_id === subject.parent_id : target.kind === 'caption',
        'ASSOCIATION', id, 'target_ids', 'wrong target kind or table');
    }
  }
}

export interface SuppliedRepresentation {
  format: string;
  source_path: string;
  origin_kind: string;
...
const IMPORT_TABLES = ['assets', 'provenance', 'bindings', 'objects', 'associations'] as const;
const LIST_FIELDS = ['input_refs', 'output_refs', 'binding_ids', 'target_ids'];
export function representationInputPath(value: unknown): string {
  requireMaterial(typeof value === 'string' && value.length > 0 && !/^[\\/]/u.test(value)
    && !/[\\:\0]/u.test(value) && value.split('/').every((part) => part && part !== '.' && part !== '..'),
  'CAPTURE_HASH', REPRESENTATION_SUFFIX, 'input_path', 'expected normalized relative file path');
  return value;
}
export function parseSuppliedRepresentation(raw: Buffer): SuppliedRepresentation {
  let parsed: unknown;
  try { parsed = parseStrictJson(raw, true); } catch {
  ...
  for (const field of ['origin_kind', 'extraction_surface', 'state', 'reason']) requireMaterial(typeof parsed[field] === 'string',
    'FORMAT', REPRESENTATION_SUFFIX, field, 'expected string');
  for (const key of IMPORT_TABLES) {
    ...
    for (const row of rows) {
      ...
      for (const field of header) {
        if (LIST_FIELDS.includes(field)) requireMaterial(Array.isArray(row[field]), 'FORMAT', String(row[header[0]]), field, 'expected JSON array');
        else if (field === 'coordinates') requireMaterial(record(row[field]), 'FORMAT', String(row[header[0]]), field, 'expected coordinate object');
        else requireMaterial(typeof row[field] === 'string', 'FORMAT', String(row[header[0]]), field, 'expected table scalar string');
      }
      const id = String(row[header[0]]);
      const family = ({ assets: 'AST', provenance: 'RPR', bindings: 'BND', objects: 'OBJ', associations: 'ASC' } as const)[key];
      requireMaterial(idFor(id, family) && !seen.has(id), 'IDENTITY', id, header[0], 'invalid or duplicate local ID');
      seen.add(id);
      if (key === 'assets') representationInputPath(row.input_path);
      if (key === 'provenance') requireMaterial(row.type !== 'capture', 'PROVENANCE', id, 'type', 'capture is supplied by importer');
    }
  }
  const descriptor = parsed as unknown as SuppliedRepresentation;
...
export interface RepresentationCaptureInput {
  source_id: string;
  bytes: Buffer;
  descriptor?: Buffer;
  assets?: ReadonlyMap<string, Buffer>;
}
export interface RepresentationCapture {
  inventory: MaterialInventory;
  assets: Map<string, Buffer>;
  sourceSchemes: Map<string, 'md-lines' | 'opaque-bytes'>;
}
function tableRecord(name: InventoryTable, values: Record<string, string>): MaterialRow {
  return Object.fromEntries(MATERIAL_HEADERS[name].map((key) => [key, values[key]]));
}
function replaceReference(value: unknown, map: Record<string, string>): unknown {
  if (typeof value === 'string') return map[value] || value;
  if (Array.isArray(value)) return value.map((v) => replaceReference(v, map));
  return value;
}
function importedRow(name: typeof IMPORT_TABLES[number], row: Record<string, unknown>, rep: string, map: Record<string, string>): MaterialRow {
  const result: Record<string, string> = { representation_id: rep };
  for (const [field, value] of Object.entries(row)) {
    if (field === 'input_path') {
      ...
      const c = Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([key, v]) =>
        [key, key.endsWith('_ids') || key === 'page_id' ? replaceReference(v, map) : v]));
      result[field] = JSON.stringify(c);
    } else if (LIST_FIELDS.includes(field)) {
      const refs = replaceReference(value, map) as string[];
      result[field] = JSON.stringify(['input_refs', 'output_refs'].includes(field) ? [...refs].sort() : refs);
    } else if (field.endsWith('_id')) result[field] = String(replaceReference(value, map));
    else result[field] = field === 'exact_bytes_base64' && value === '' ? '""' : String(value);
  }
  return tableRecord(name, result);
}
export function prepareRepresentationCapture(inputs: readonly RepresentationCaptureInput[]): RepresentationCapture {
  const inventory = emptyMaterialInventory(), assets = new Map<string, Buffer>();
  const sourceSchemes = new Map<string, 'md-lines' | 'opaque-bytes'>();
  const counters: Record<string, number> = {};
  const allocate = (family: string): string => `${family}-${String(counters[family] = (counters[family] || 0) + 1).padStart(4, '0')}`;
  const carriers = new Map<string, MaterialCarrier>();
  for (const input of inputs) {
    ...
    for (const id of locals) localMap[id] = allocate(id.split('-')[0]);
    ...
    for (const [id, bytes, role] of [[declarationId, input.descriptor, 'structure-export'], [mapId, mapBytes, 'render-log']] as const) {
      ...
      assets.set(locus, bytes); carriers.set(id, { bytes, encoding: 'utf8', representation_id: rep });
    }
    const upstream: string[] = [];
    for (const name of IMPORT_TABLES) for (const row of [...descriptor[name]].sort((a, b) => String(Object.values(a)[0]).localeCompare(String(Object.values(b)[0]), 'en'))) {
      ...
      if (name === 'assets') {
        ...
        requireMaterial(bytes && bytes.length === Number(imported.byte_length) && materialHash(bytes) === imported.content_hash,
          'CAPTURE_HASH', rowId(imported), 'content_hash/byte_length', 'supplied asset bytes disagree');
        assets.set(imported.locus, bytes);
        carriers.set(rowId(imported), { bytes, encoding: imported.encoding as 'utf8' | 'opaque', representation_id: rep });
        if (imported.role === 'upstream-capture') upstream.push(rowId(imported));
      }
    }
...
function importedCorrespondence(context: RepresentationContext): void {
  const inv = context.inventory;
  for (const rep of inv.representations) {
    const capture = find(inv.provenance, rep.provenance_id);
    if (capture.declaration_asset_id === 'none') continue;
    ...
    requireMaterial(exactKeys(map, ['format', 'source_id', 'representation_id', 'capture_id', 'declaration_asset_id', 'id_map'])
      ...
      && map.representation_id === rowId(rep) && map.capture_id === rowId(capture) && record(map.id_map),
    'INVENTORY', rowId(rep), 'ID map', 'invalid map identity');
    const localMap = map.id_map as Record<string, string>;
    requireMaterial(Object.keys(localMap).join('\0') === Object.keys(localMap).sort().join('\0')
      ...
      && Object.entries(localMap).every(([a, b]) => typeof b === 'string' && idFor(b, a.split('-')[0])),
    'INVENTORY', rowId(rep), 'ID map', 'mapping must be sorted bijection preserving families');
    ...
    for (const field of ['origin_kind', 'extraction_surface', 'state', 'reason'] as const)
    ...
    for (const key of IMPORT_TABLES) {
      ...
      for (const row of expected) requireMaterial(JSON.stringify(row) === JSON.stringify(actual.find((a) => rowId(a) === rowId(row))),
        'INVENTORY', rowId(row), key, 'imported fields differ from raw declaration');
    }
  }
}
export function readRepresentationContext(model: RunModel): RepresentationContext {
  ...
  const context: RepresentationContext = { inventory, inventoryHash: materialHash(raw), carriers: new Map(), uses: [] };
  ...
  for (const source of model.corpus.sources) {
    ...
    requireMaterial(rep.extraction_surface === 'utf8-text' ? s.scheme === 'md-lines' && utf8(bytes) : s.scheme === 'opaque-bytes',
      'STATE', s.sourceId, 'scheme', 'capture surface/encoding/scheme mismatch');
  ...
  for (const asset of inventory.assets) {
    ...
    requireMaterial(new RegExp(`^${REPRESENTATION_ASSET_PATH}/${id}\\.[a-z0-9]{1,12}$`, 'u').test(asset.locus) && !loci.has(asset.locus),
      'CAPTURE_HASH', id, 'locus', 'asset locus must match identity exactly');
    ...
    requireMaterial(!['structure-export', 'rendered-text', 'render-log'].includes(asset.role) || asset.encoding === 'utf8',
      'PROVENANCE', id, 'encoding', 'textual export requires UTF-8');
    ...
    context.carriers.set(id, { bytes, encoding: asset.encoding as 'utf8' | 'opaque', representation_id: asset.representation_id });
  }
...
export function validateMaterialUseInput(value: unknown, canonical = true): MaterialUseInput {
  const keys = (item: unknown, fields: string[]): item is Record<string, unknown> =>
    canonical ? exactKeys(item, fields) : record(item) && JSON.stringify(Object.keys(item).sort()) === JSON.stringify([...fields].sort());
  requireMaterial(keys(value, ['requirements', 'use_state', 'fidelity_claim', 'limitation_refs', 'reason']),
    'UNDECLARED_FEATURE', REPRESENTATION_USE_PATH, 'material_use', 'exact material use fields required');
  ...
  for (const req of value.requirements) {
    requireMaterial(keys(req, ['object_id', 'feature', 'binding_ids']) && typeof req.object_id === 'string'
      && idFor(req.object_id, 'OBJ') && (MATERIAL_FEATURES as readonly unknown[]).includes(req.feature)
      && Array.isArray(req.binding_ids) && req.binding_ids.every((id) => typeof id === 'string' && idFor(id, 'BND'))
      && new Set(req.binding_ids).size === req.binding_ids.length,
    'UNDECLARED_FEATURE', REPRESENTATION_USE_PATH, 'requirements', 'invalid requirement tuple');
  ...
  requireMaterial(typeof value.reason === 'string' && (value.use_state === 'usable' ? value.reason === 'none'
    : value.reason.trim() && value.reason !== 'none' && value.limitation_refs.length > 0),
  'STATE', REPRESENTATION_USE_PATH, 'reason', 'use state requires honest reason and limitations');
  return {
    requirements: value.requirements.map((req) => {
      const r = req as MaterialRequirement;
      return { object_id: r.object_id, feature: r.feature, binding_ids: r.binding_ids };
    }),
    use_state: value.use_state, fidelity_claim: value.fidelity_claim as MaterialUseInput['fidelity_claim'],
    limitation_refs: value.limitation_refs as string[], reason: value.reason,
  };
}
export function materialFeatureAvailable(context: RepresentationContext, requirement: MaterialRequirement): boolean {
  ...
  requireMaterial(JSON.stringify(owned.filter((id) => requirement.binding_ids.includes(id))) === JSON.stringify(requirement.binding_ids),
    'UNDECLARED_FEATURE', rowId(object), 'binding_ids', 'requirements must preserve declared binding order');
  const textual = (o: MaterialRow): boolean => objectBindings(context, o).length > 0
...
function useInput(row: MaterialRow): MaterialUseInput {
...
export function representationUseNeedsReview(context: RepresentationContext, input: MaterialUseInput): boolean {
  return input.requirements.some((req) => req.feature !== 'text-bytes' || req.binding_ids.some((id) =>
    find(context.inventory.bindings, id).byte_role === 'retained-asset-bytes'));
}
export function representationUseSubjectJson(model: RunModel, context: RepresentationContext, row: MaterialRow): string {
  ...
  if (row.subject_kind === 'PKT') {
    ...
    subject = JSON.parse(relationReviewSubjectJson(r.values)) as unknown;
  } else {
  ...
  for (const field of MATERIAL_HEADERS.uses.slice(1, 11)) fields[field] =
  ...
  const hashes = basis.map((id) => {
    ...
    requireMaterial(records.length === 1 && records[0].values.evidenceState === 'exact',
      'USE_CLOSURE', rowId(row), 'basis_packet_ids', 'each packet needs its exact evidence record');
...
export function representationUseDigest(model: RunModel, context: RepresentationContext, row: MaterialRow): string {
  return materialHash(representationUseSubjectJson(model, context, row));
}
function useSources(model: RunModel, context: RepresentationContext, row: MaterialRow): string[] {
  if (row.subject_kind === 'OBJ') return [find(context.inventory.representations,
    find(context.inventory.objects, row.subject_id).representation_id).source_id];
...
export function validateRepresentationUse(model: RunModel, context: RepresentationContext, row: MaterialRow, review = true): void {
  ...
  if (row.subject_kind === 'PKT') requireMaterial(JSON.stringify(basis) === JSON.stringify([row.subject_id]), 'USE_CLOSURE', id, 'basis_packet_ids', 'packet basis must be itself');
  if (row.subject_kind === 'CC') {
    const c = model.claims.find((c) => c.values.claimId === row.subject_id);
    requireMaterial(c && JSON.stringify(basis) === JSON.stringify(c.values.packets.split(',').map((p) => p.trim())),
      'USE_CLOSURE', id, 'basis_packet_ids', 'claim provenance/order mismatch');
  }
  if (row.subject_kind === 'REL') {
    const relation = parseRelations(model).rows.find((r) => r.values.relationId === row.subject_id);
    requireMaterial(relation && JSON.stringify(basis) === JSON.stringify(parsePacketBasis(relation.values.basisPacketIds).ids),
      'USE_CLOSURE', id, 'basis_packet_ids', 'relation basis/order mismatch');
  }
  requireMaterial(row.subject_kind === 'OBJ' || input.use_state === 'usable',
    'USE_CLOSURE', id, 'use_state', 'CANNOT_DETERMINE candidates require OBJ receipts, not canonical packets/claims/relations');
  validateUseRequirements(model, context, input, sources, basis, row.subject_kind, row.subject_id, id);
  requireMaterial(row.review_subject_digest === representationUseDigest(model, context, row),
    'USE_CLOSURE', id, 'review_subject_digest', 'reviewed immutable subject differs');
  ...
  if (row.reviewed_by !== 'none') {
    ...
    let verdict;
    try { verdict = parseStructuredVerifierRecord(readMaterialFile(model.runDir, path), path); } catch {
    ...
    if (requiresReview) requireMaterial(verdict.lens === 'L2F' && verdict.stage === row.owner_stage,
      'USE_CLOSURE', id, 'reviewed_by', 'L2F stage/charter mismatch');
  }
}
/** Shared declaration rules; degraded subjects have no canonical packet basis. */
function validateUseRequirements(model: RunModel, context: RepresentationContext, input: MaterialUseInput,
  sources: string[], basis: string[], kind: string, subjectId: string, id: string): void {
  const inv = context.inventory;
  ...
  for (const requirement of input.requirements) {
    ...
    for (const bindingId of requirement.binding_ids) {
      ...
      if (kind !== 'OBJ' && kind !== 'degraded-packet' && b.byte_role === 'frozen-source-bytes') {
        ...
        for (const packetId of basis) for (const fragment of model.exactEvidence.fragments.filter((f) =>
          f.values.packetId === packetId && f.values.sourceId === rep.source_id)) {
        ...
        requireMaterial(defaultWhole ? spans.length > 0 : intervalCovered(Number(b.start_byte), Number(b.end_byte), spans),
          'USE_CLOSURE', id, 'binding_ids', 'SRC binding outside exact packet-basis union');
        if (kind === 'REL' && !defaultWhole) {
          ...
          if (relation.recordState === 'asserted' && relation.targetKind === 'source-locus'
            ...
            requireMaterial(target?.startByte !== null && target?.endByte !== null && target
              && Number(b.start_byte) >= target.startByte! && Number(b.end_byte) <= target.endByte!,
            'USE_CLOSURE', id, 'binding_ids', 'selected target SRC binding outside reopened relation target span');
          }
        }
      }
    }
...
export function representationLimitations(model: RunModel, context: RepresentationContext): MaterialRow[] {
  const rows: MaterialRow[] = [];
  for (const key of ['representations', 'objects', 'associations'] as const) for (const r of context.inventory[key]) {
  ...
  for (const use of context.uses.filter((r) => r.use_state === 'CANNOT_DETERMINE')) {
    for (const source of useSources(model, context, use)) rows.push({
...
export const LIMITATION_HEADERS = ['limitation_ref', 'source_id', 'representation_state', 'reason'] as const;
export function representationUseClosureHash(model: RunModel): string | null {
  ...
  requireMaterial(c1.length === 1 && c1[0].stage === 'S4' && lines.length === 1 && /^representation_use_closure_hash: sha256:[0-9a-f]{64}\r?$/u.test(lines[0]),
    'FROZEN_WRITE', 'run-log.md', 'representation_use_closure_hash', 'one exact C1 seal required');
  const index = events.indexOf(c1[0]);
  requireMaterial(log.lines.slice(c1[0].line, events[index + 1]?.line ? events[index + 1].line - 1 : undefined).includes(lines[0]),
    'FROZEN_WRITE', 'run-log.md', 'representation_use_closure_hash', 'seal outside C1 event');
  return lines[0].trim().slice('representation_use_closure_hash: '.length);
}
export function validateRepresentationRun(model: RunModel): RepresentationContext {
  ...
  if (frozen || sealLines.length) requireMaterial(sealLines.length === 1
    && sealLines[0] === `- representation_inventory_hash: ${materialHash(readMaterialFile(model.runDir, REPRESENTATION_PATH))}`,
  'CAPTURE_HASH', 'run-manifest.md', 'representation_inventory_hash', 'missing/mismatched inventory seal');
  ...
  ] as const) for (const id of ids) requireMaterial(subjects.has(`${kind}:${id}`), 'USE_CLOSURE', id, 'receipt', 'canonical subject has no receipt');
  ...
  if (closure) requireMaterial(closure === materialHash(readMaterialFile(model.runDir, REPRESENTATION_USE_PATH)),
    'FROZEN_WRITE', REPRESENTATION_USE_PATH, 'closure_hash', 'uses changed after retained C1');
  if (['ASSEMBLED', 'VERIFIED', 'ACCEPTED', 'PROJECTING', 'PROJECTION-ACCEPTED'].some((s) => reachedState(model, s))) {
    ...
    requireMaterial(matching.length === 1 && JSON.stringify(matching[0].header) === JSON.stringify(LIMITATION_HEADERS),
      'USE_CLOSURE', 'precis.md', 'section 17', 'one canonical limitation table required');
...
export function assertRepresentationExtractionSupported(context: RepresentationContext): void {
  requireMaterial(!context.inventory.representations.some((r) => r.extraction_surface === 'opaque'),
    'UNSUPPORTED_EXTRACTION_SURFACE', REPRESENTATION_PATH, 'extraction_surface', 'opaque capture cannot progress to S2; retain limitation or use a successor run');
}
export interface MaterialFileWrite { path: string; before_hash: string; after_base64: string; after_hash: string }
export interface MaterialWritePlan { key: string; inventory_hash: string; writes: MaterialFileWrite[] }
export function requiresMaterialUsePlan(path: string): boolean {
  return ['ledgers/packet-index.md', 'ledgers/claim-inventory.md', 'ledgers/relations.md', REPRESENTATION_USE_PATH].includes(path);
}
export function assertMaterialWriteWindow(model: RunModel, stage: string, ownerStage: string): void {
  ...
  requireMaterial(stage === ownerStage && ['S2', 'S3', 'S4'].includes(stage) && representationUseClosureHash(model) === null,
    'FROZEN_WRITE', REPRESENTATION_USE_PATH, 'owner_stage', 'material write window closed');
}
export function materialSubjectWritePaths(kind: string): readonly string[] {
...
export function validateMaterialPlanIdentity(plan: MaterialWritePlan, row: MaterialRow | null, stage: string): void {
  ...
  requireMaterial(row ? row.owner_stage === stage
    ...
    : stage === 'S4' && /^representation-use-closure:sha256:[0-9a-f]{64}$/u.test(plan.key),
  'FROZEN_WRITE', plan.key, 'identity', 'transaction key or stage differs from reserved subject');
...
export function planRepresentationUseWrite(options: {
  model: RunModel; proposedModel: RunModel; row: MaterialRow; subjectWrites: MaterialFileWrite[]; stage: string;
}): MaterialWritePlan {
  ...
  requireMaterial(!context.uses.some((r) => rowId(r) === rowId(row)
    || (row.subject_kind !== 'OBJ' && r.subject_kind === row.subject_kind && r.subject_id === row.subject_id)),
  'USE_CLOSURE', rowId(row), 'reservation', 'canonical identity already has different use');
  ...
  for (const write of options.subjectWrites) {
    requireMaterial(materialSubjectWritePaths(row.subject_kind).includes(write.path),
      'FROZEN_WRITE', write.path, 'subjectWrites', 'outside material subject write surface');
...
export function validateMaterialProducerReturn(value: unknown): void {
  if (!record(value)) return;
  const inspect = (item: unknown): void => {
    if (!record(item)) return;
    if ('material_use' in item) validateMaterialUseInput(item.material_use, false);
    if (Array.isArray(item.flags)) materialRefusalFlags(item.flags.filter((v): v is string => typeof v === 'string'));
    for (const child of Object.values(item)) {
      if (Array.isArray(child)) child.forEach(inspect);
      else if (record(child)) inspect(child);
    }
  };
  inspect(value);
  if (Array.isArray(value.material_findings)) for (const finding of value.material_findings) {
    requireMaterial(record(finding) && JSON.stringify(Object.keys(finding).sort()) === '["material_use","object_id"]'
      && typeof finding.object_id === 'string' && idFor(finding.object_id, 'OBJ'),
      'UNDECLARED_FEATURE', 'worker return', 'material_findings', 'malformed material finding');
    requireMaterial(validateMaterialUseInput(finding.material_use, false).use_state === 'CANNOT_DETERMINE',
      'UNDECLARED_FEATURE', finding.object_id, 'material_findings', 'finding must preserve non-affirmative candidate');
  }
}
/** A recognized downstream refusal halts writing; retained raw return is the evidence. */
export function assertMaterialReturnWritable(value: unknown): void {
  const visit = (item: unknown): void => {
    if (!record(item)) return;
    if (Array.isArray(item.flags)) requireMaterial(materialRefusalFlags(item.flags.filter((v): v is string => typeof v === 'string')).length === 0,
      'UNDECLARED_FEATURE', 'worker return', 'flags', 'CANNOT_DETERMINE requires unresolved handling or halt before writing');
    for (const child of Object.values(item)) {
...
export function assertMaterialUseProduced(value: unknown, row: MaterialRow): void {
  ...
  const visit = (item: unknown): void => {
    ...
    if ('material_use' in item) {
      ...
      if (JSON.stringify(actual) === JSON.stringify(expected)
        && (row.subject_kind !== 'OBJ' || item.object_id === row.subject_id)) found = true;
    }
    for (const child of Object.values(item)) {
      if (Array.isArray(child)) child.forEach(visit);
      else visit(child);
...
export function assertMaterialReviewUpheld(value: unknown, digest: string): void {
  requireMaterial(materialReviewUseState(value) !== 'CANNOT_DETERMINE', 'USE_CLOSURE', 'L2F', 'use_state',
    'CANNOT_DETERMINE blocks affirmative use; retain the missing-material finding');
...
export function materialReviewUseState(value: unknown): 'usable' | 'CANNOT_DETERMINE' | null {
  requireMaterial(record(value) && ['upheld', 'refuted', 'cannot-determine'].includes(String(value.verdict)),
    'STATE', 'L2F', 'verdict', 'unknown material review verdict');
  return value.verdict === 'cannot-determine' ? 'CANNOT_DETERMINE' : value.verdict === 'upheld' ? 'usable' : null;
}

export function materialFindingRows(model: RunModel, value: unknown, stage: string, establishedBy: string): MaterialRow[] {
  validateMaterialProducerReturn(value);
  const context = validateRepresentationRun(model);
  if (!record(value) || !Array.isArray(value.material_findings)) return [];
  let next = Math.max(0, ...context.uses.map((r) => Number(r.use_id.slice(4))));
  return value.material_findings.flatMap((entry) => {
    const finding = entry as Record<string, unknown>, input = validateMaterialUseInput(finding.material_use, false);
    ...
    if (context.uses.some((r) => r.subject_kind === 'OBJ' && r.subject_id === row.subject_id
      && r.review_subject_digest === row.review_subject_digest)) return [];
    validateRepresentationUse(model, context, row);
    return [row];
  });
}
...
export function representationReviewView(model: RunModel, context: RepresentationContext, row: MaterialRow): string {
  validateRepresentationUse(model, context, row, false);
  return materialReviewView(model, context, useInput(row), useSources(model, context, row),
    list(row.basis_packet_ids, rowId(row), 'basis_packet_ids'), JSON.parse(representationUseSubjectJson(model, context, row)),
    `representation-use-subject:${row.review_subject_digest}`);
}
/** One source-bound material view, without an OBJ identity or a synthetic USE row. */
export function degradedPacketMaterialView(model: RunModel, context: RepresentationContext, sourceId: string,
  producerBindingHash: string, materialUse: MaterialUseInput): string {
  requireMaterial(hasRunCapability(model.manifest?.runFormatVersion || '', 'orchestrator-work-transitions'),
    'FORMAT', 'degraded-packet', 'run_format', 'orchestrator-work-transitions required');
...
function materialReviewView(model: RunModel, context: RepresentationContext, input: MaterialUseInput,
  ...
  const add = (id: string, material: boolean): void => {
    const object = find(inv.objects, id);
    if (material) list(object.binding_ids, id, 'binding_ids').forEach((b) => bindings.add(b));
    if (objects.has(id)) return;
    objects.add(id);
    if (object.parent_id !== 'none') add(object.parent_id, false);
    const coords = coordinates(object);
    for (const value of Object.values(coords)) for (const ref of Array.isArray(value) ? value : [value]) {
      if (typeof ref === 'string' && idFor(ref, 'OBJ')) add(ref, true);
    }
  ...
  const assets = new Set<string>(), provenanceIds = new Set<string>();
  ...
  const addProvenance = (id: string): void => {
    if (provenanceIds.has(id)) return;
    ...
    for (const field of ['parameters_asset_id', 'declaration_asset_id']) {
      if (provenance[field] !== 'none') addAsset(provenance[field]);
    }
...
export function materialRefusalFlags(flags: readonly string[]): Array<{ id: string; feature: string }> {
  return flags.filter((flag) => flag.startsWith('CANNOT_DETERMINE:source-representation:')).map((flag) => {
    ...
    requireMaterial(match && (MATERIAL_FEATURES as readonly string[]).includes(match[2]), 'UNDECLARED_FEATURE', 'worker return', 'flags', 'invalid material refusal');
    return { id: match[1], feature: match[2] };
  });
}

export function selectRepresentationInventory(inventory: MaterialInventory, sourceIds: readonly string[]): MaterialInventory {
