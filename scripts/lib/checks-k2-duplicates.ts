...
import { hasRunCapability, SUPPORTED_RUN_FORMAT_VERSIONS, walkFiles, type RunModel } from './run-model.ts';
import type { ResultCollector } from './results.ts';
import { DUPLICATE_PATH, validateDuplicateRun } from './duplicate-review.ts';

export function duplicateArtifactsPresent(model: RunModel): boolean {
  return [DUPLICATE_PATH, 'verification/harness/duplicate-discovery', 'verification/harness/duplicate-subjects',
    ...
    || (existsSync(join(model.runDir, 'control/worker-bundles')) && walkFiles(join(model.runDir, 'control/worker-bundles'))
      .filter((path) => path.endsWith('/contracts/output.json')).some((path) => {
        try { return JSON.parse(readFileSync(path, 'utf8')).contract_format === 'aleph-duplicate-output-contract/v1'; } catch { return false; }
      }))
...
export function runK2Duplicates(results: ResultCollector, model: RunModel): void {
  const version = model.manifest?.runFormatVersion || '', active = hasRunCapability(version, 'duplicate-overlap-review');
  if (!active && !duplicateArtifactsPresent(model) && (!version || (SUPPORTED_RUN_FORMAT_VERSIONS as readonly string[]).includes(version))) return;
  results.run('K2.20', 'duplicate comparison and admission accounting (STRUCTURAL ONLY)', () => {
    if (!active) throw new Error(`DUP_COMPATIBILITY ${version}: operative duplicate artifacts require duplicate-overlap-review`);
