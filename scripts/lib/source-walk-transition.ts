...
import { sourceFilePath } from './check-helpers.ts';
import { runK2 } from './checks-k2.ts';
import { parseTables, parseBulletFields } from './markdown.ts';
import { hasRunCapability, parseSourceWalk, type RunModel, type RunDocument } from './run-model.ts';
...
export interface CompletionProjection {
  source_id: string;
  progression: 'absent-to-blocked' | 'blocked-to-blocked' | 'blocked-to-complete' | 'complete-no-op' | 'blocked-no-op';
  before_row: string | null;
  after_row: string;
}
export interface SourceWalkProjection {
...
function requireWalk(condition: unknown, why: string): asserts condition {
  if (!condition) throw new Error(`WORK_SOURCE_COMPLETION ${why}`);
}
function gapIds(value: string): string[] {
...
export function projectSourceWalk(model: RunModel, text: string): RunModel {
...
function withCompletionRows(model: RunModel, rows: readonly string[]): RunModel {
...
function k214(model: RunModel): string[] {
 ...
 */
export function deriveSourceWalkCompletion(before: RunModel, history: RunModel): SourceWalkProjection {
  ...
  requireWalk(new Set(history.sourceWalk.completions.map((entry) => entry.values.sourceId)).size === history.sourceWalk.completions.length,
    'duplicate proposed completion row');
  ...
  for (const source of orderedSources) {
    ...
    if (previous) {
      requireWalk(previous.values.sourceHash === hash && previous.values.sourceLengthBytes === String(bytes.length),
        `${id} frozen hash/length changed`);
      ...
      if (previous.values.completionState === 'complete') {
        requireWalk(current.values.cursorId === previous.values.finalCursorId
          && reviews.join(', ') === (previous.values.gapReviewIds === 'none' ? '' : previous.values.gapReviewIds),
        `${id} completed projection is terminal`);
  ...
  for (const [index, projection] of projections.entries()) {
    if (projection.progression === 'absent-to-blocked' || projection.progression === 'complete-no-op') continue;
