...
import { parseLineage, lineageCurrentPacketIds, LINEAGE_TABLE_HEADER } from './lineage.ts';
import { deriveSourceWalkCompletion, derivePendingEventCommitment, validateSourceWalkCompletionWrite,
  projectSourceWalk, type SourceWalkProjection } from './source-walk-transition.ts';
import {
...
export type WorkStage = 'S0' | 'S1' | 'S2' | 'S3' | 'S4' | 'S5';
...
export interface WorkCall {
  ...
  task_line: string;
  allowlist: string[];
  producer_dependency: string | null;
...
export interface WorkFieldOrigin {
  artifact: string;
  field: string;
  from: { kind: 'accepted'; call_id: string; selector: string }
    | { kind: 'canonical'; path: string; selector: string }
    | { kind: 'rule'; rule: string };
}
export interface WorkFileEffect {
  path: string;
...
export interface CriteriaSubject {
  format: 'aleph-criteria-subject/v1';
  run_id: string;
  criteria_digest: string;
  criteria_bytes_base64: string;
  sample_proposal: {
    path: typeof CRITERIA_SAMPLE_PROPOSAL_PATH;
    digest: string;
    claim: 'bounded-proposal-not-representative-coverage';
  };
  samples: CriteriaSample[];
}

export function workDigest(bytes: Buffer | string): string {
  return `sha256:${createHash('sha256').update(bytes).digest('hex')}`;
}
export function workJson(value: unknown): Buffer {
  const bytes = canonicalJsonBytes(value);
  parseStrictJson(bytes);
  return bytes;
}
export function workFailure(code: string, detail: string): never {
  throw new Error(`${code}: ${detail}`);
}
export function assertWork(condition: unknown, code: string, detail: string): asserts condition {
  if (!condition) workFailure(code, detail);
}
...
function obligation(stage: WorkStage, dod: string, operation: string, subject: string, bytes: Buffer): WorkObligation {
  return { stage, subphase: stage, dod, operation, subject_id: subject, subject_digest: workDigest(bytes) };
}
function effect(model: RunModel, path: string, bytes: Buffer): WorkFileEffect {
...
function appendRows(bytes: Buffer, firstHeader: string, rows: readonly (readonly string[])[]): Buffer {
...
const CLAIM_HEADERS = ['claim_id', 'normalized claim', 'packets', 'sources', 'claim_type',
  'disposition', 'rationale', 'judged_by', 'verified', 'status'] as const;
interface S3Preparation {
  format: 'aleph-s3-work-preparation/v1'; call_id: string; origin_semantic_id: string;
}
interface S3Capture {
  format: 'aleph-s3-work-capture/v1'; call_id: string; origin_semantic_id: string; raw_digest: string;
  context_id: string; producer_context_id: string | null; receipt_digest: string; simulation: boolean;
  selectors: Array<{ output_kind: 'claim-candidate' | 'no-claim-candidate' | 'material-candidate'; output_index: string;
    reserved_claim_id: string | null; binding_path: string }>;
}
function s3Captures(model: RunModel): S3Capture[] {
  return model.files.filter((entry) => entry.relativePath.startsWith(S3_CAPTURES)).map((entry) => {
    ...
    for (const selector of value.selectors) {
      ...
      assertWork(['claim-candidate', 'no-claim-candidate', 'material-candidate'].includes(String(selector.output_kind))
        ...
        && selector.binding_path === `control/semantic-producer-bindings/${value.call_id}/${selector.output_kind}-${selector.output_index}.json`,
      'WORK_CAPTURE', 'original normalizer selector binding');
    }
    return value as unknown as S3Capture;
  });
}
function semanticDependencies(model: RunModel, semanticId: string): string[] {
  ...
  const binding = parseStrictJson(readFileSync(join(model.runDir, row.producer_receipt_ref.split('@')[0]))) as { call_id: string };
  return [binding.call_id, ...ledger.assignments.filter((entry) => entry.semantic_id === semanticId).map((entry) =>
    (parseStrictJson(required(model, entry.assignment_path)) as { invocation_id: string }).invocation_id)];
}
function s3Preparation(model: RunModel, id: string): S3Preparation {
...
function selectS3Work(model: RunModel): NextWork {
  ...
  for (const row of ledger.subjects.filter((entry) => entry.owner_stage === 'S2')) {
    ...
    if (subject.output_binding.kind !== 'packet-group' || !subject.output_binding.packet_ids.some((id) => current.has(id))) continue;
    if (captures.some((capture) => capture.origin_semantic_id === row.semantic_id)) continue;
    const reviews = ledger.results.filter((entry) => entry.semantic_id === row.semantic_id)
      .map((entry) => parseStrictJson(required(model, entry.result_path)) as unknown as SemanticResult);
    if (!reviews.length || reviews.some((entry) => entry.verdict !== 'upheld')) return {
    ...
    if (!file(model, `${S3_PREPARATIONS}${prep.call_id}.json`)) return { kind: 'local',
    ...
    const view = semanticProducerView(model, 'normalizer', 'S3',
...
function deriveS3Capture(model: RunModel, work: Extract<NextWork, { kind: 'worker' }>, accepted: WorkValue): WorkFileEffect[] {
  ...
  const selectors: S3Capture['selectors'] = [];
  for (const [array, kind] of [['claims', 'claim-candidate'], ['no_claim_packets', 'no-claim-candidate'], ['material_findings', 'material-candidate']] as const) {
    for (const index of (value[array] as unknown[]).keys()) {
  ...
  const capture: S3Capture = { format: 'aleph-s3-work-capture/v1', call_id: accepted.call_id, origin_semantic_id: work.obligation.subject_id,
  ...
  const uses = materialFindingRows(model, accepted.value, 'S3', accepted.call_id).map((row) => {
    row.use_id = nextId('USE', idsUsed); idsUsed.push(row.use_id);
...
function reserveS3Subject(model: RunModel, work: Extract<NextWork, { kind: 'local' | 'worker' }>): Pick<WorkTransition, 'effects' | 'semantic' | 'simulation'> {
  ...
  const raw = capturedValue(model, capture).value as Record<string, WorkerJsonValue>;
  const entry = (raw.semantic_units as unknown as SemanticEntry[]).find((entry) => entry.output_kind === kind && entry.output_index === Number(index))!;
  ...
  if (output.kind === 'claim') {
    const row: MaterialRow = { use_id: nextId('USE', context.uses.map((row) => row.use_id)), owner_stage: 'S3',
...
function s3Admission(model: RunModel, subject: SemanticSubject, callId: string): { effects: WorkFileEffect[]; refs: string[] } {
  ...
  if (output.kind === 'no-claim') {
    ...
    return { refs: [output.packet_id, id], effects: [effect(model, 'ledgers/lineage.md',
      appendRows(required(model, 'ledgers/lineage.md'), LINEAGE_TABLE_HEADER[0],
        [[id, 'S3', 'no-claim', output.packet_id, 'none', output.basis, callId]]))] };
  }
...
function capturedValue(model: RunModel, capture: S2Capture | S3Capture): WorkValue {
...
function selectSemanticWork(model: RunModel, capture: S2Capture | S3Capture): NextWork | null {
  ...
  for (const selector of capture.selectors) {
    ...
    if (!reserved.length) return { kind: 'local', accepted_dependencies: [capture.call_id],
      obligation: obligation(stage, `${stage}.L2S.reserve`, `sem.reserve-${stage}`,
...
function semanticTransition(model: RunModel, work: Extract<NextWork, { kind: 'local' | 'worker' }>,
  ...
  let subject: SemanticSubject, capture: S2Capture | S3Capture, producerCall: string;
  ...
  if (operation === 'sem.reserve-S3') return reserveS3Subject(model, work);
  ...
  producerCall = binding.call_id; capture = [...s2Captures(model), ...s3Captures(model)].find((entry) => entry.call_id === producerCall)!;
...
function selectS2Work(model: RunModel): NextWork {
  ...
  for (const source of model.corpus.sources) {
    ...
    if (event) return { kind: 'local', accepted_dependencies: [event.values.producerInvocationId],
      obligation: obligation('S2', 'S2.primary-walk.event-commitment', 's2.commit-event', event.values.eventId,
...
export function validateCriteriaReview(value: unknown, subject: CriteriaSubject): void {
  ...
  value.judgments.forEach((judgment: unknown, index: number) => {
    ...
    if (judgment.candidacy === 'candidate') assertWork(judgment.criterion_refs.some((ref) => admission.includes(String(ref))), 'WORK_REFERENCE', 'candidate criterion');
    if (judgment.candidacy === 'excluded') assertWork(judgment.criterion_refs.some((ref) => exclusion.includes(String(ref))), 'WORK_REFERENCE', 'exclusion criterion');
  });
  if (value.verdict === 'upheld') assertWork(value.sample_adequacy === 'adequate'
    && value.judgments.every((entry) => (entry as Record<string, unknown>).candidacy !== 'cannot-determine'), 'WORK_CONTRACT', 'indeterminate criteria cannot be upheld');
}
 ...
 */
export function criteriaSampleProposal(model: RunModel, criteria: Buffer, proposalBytes = required(model, CRITERIA_SAMPLE_PROPOSAL_PATH)): CriteriaSubject {
  const samples: CriteriaSample[] = [];
...
export function selectNextWork(model: RunModel, execution: WorkExecution): NextWork {
  ...
  if (execution.stage === 'S1') {
    if (!file(model, CRITERIA_SAMPLE_PROPOSAL_PATH)) {
      ...
      if (!existsSync(input)) return { kind: 'proposal', operation: 'criteria.samples', input_path: CRITERIA_SAMPLE_INPUT_PATH };
    ...
    if (!criteria) return { kind: 'worker',
    ...
    const subject = parseStrictJson(subjectBytes) as unknown as CriteriaSubject;
    ...
    for (const [index, path] of CRITERIA_REVIEW_PATHS.entries()) if (!file(model, path)) {
    ...
    const reviews = CRITERIA_REVIEW_PATHS.map((path) => parseStrictJson(required(model, path)) as unknown as WorkValue);
    for (const review of reviews) validateCriteriaReview(review.value, subject);
    assertWork(reviews[0].context_id !== reviews[1].context_id, 'WORK_REVIEW_ISOLATION', 'criteria reviewers reused context');
    const values = reviews.map((review) => review.value as Record<string, WorkerJsonValue>);
...
function intakeEffects(model: RunModel, accepted: WorkValue, now: string): { effects: WorkFileEffect[]; origins: WorkFieldOrigin[] } {
  const returned: unknown = accepted.value;
  ...
  const sources = returned.sources as unknown[];
  const rows = sources.map((value, index) => {
    ...
    text(value.admission_note, 'admission note'); text(value.dates, 'source dates');
  ...
  for (const key of ['candidate_definition', 'granularity_policy', 'normalization_conventions']) text(c[key], key);
  ...
  const admissions = c.admission.map((value, index) => {
    ...
    text(value.criterion, 'criterion'); text(value.example, 'example');
  ...
  const exclusions = c.exclusion_classes.map((value) => {
    ...
    text(value.description, 'exclusion description'); text(value.example, 'exclusion example');
...
export function deriveWorkTransition(model: RunModel, execution: WorkExecution, work: NextWork,
  accepted: WorkValue | null, now: string): WorkTransition {
  ...
  if (work.kind === 'worker') {
    ...
    if (work.obligation.operation === 's3.capture') return { ...base, family: 's3-capture',
    ...
    if (index === 1) {
      const first = parseStrictJson(required(model, CRITERIA_REVIEW_PATHS[0])) as unknown as WorkValue;
  ...
  if (work.obligation.operation === 's2.commit-event') {
    ...
    assertWork(capture && work.accepted_dependencies?.length === 1 && work.accepted_dependencies[0] === capture.call_id,
      'WORK_EVENT_BINDING', 'exact authenticated original producer dependency required');
...
export function validateDerivedWorkTransition(model: RunModel, proposedModel: RunModel, transition: WorkTransition): void {
  ...
  if (transition.source_completion) {
    assertWork(workJson(validateSourceWalkCompletionWrite(model, proposedModel)).equals(workJson(transition.source_completion)),
      'WORK_PLAN', 'source-walk transition differs from Core lifecycle derivation');
  }
