...
import {
  ...
  RELATION_FAMILIES,
  RELATION_FAMILY_TYPES,
  RELATION_FORMAT,
  RELATION_INDETERMINATE_REASONS,
  RELATION_RECORD_STATES,
  RELATION_TABLE_HEADER,
  RELATION_TYPES,
  RELATION_UNRESOLVED_REASONS,
  parsePacketBasis,
  parseRelations,
...
const CONCRETE_TARGET_KINDS = ['CC', 'PKT', 'source-locus'] as const;
...
function relationClosureRecorded(model: RunModel): boolean {
  ...
  return Boolean(model.runLog?.lines.some((line) => (
    /^##\s+.+[—-]\s+S4\s+[—-]\s+(?:exit|closed|closure)\b/i.test(line)
  )));
}

function relationStructurePresent(model: RunModel): boolean {
  return [...model.documents.values()].some((document) => (
    document !== null
    && (
...
function exactId(value: string, family: 'CC' | 'PKT' | 'SRC' | 'VER'): boolean {
...
function verifierVerdicts(model: RunModel, id: string): VerifierVerdict[] {
...
function concreteTarget(row: RelationRow): string {
...
function cycleStart(edges: ReadonlyArray<readonly [string, string]>): string | null {
...
function checkSourceLocus(
  ...
  if (scheme !== 'md-lines') {
    fail(
      `${label} source-locus scheme "${scheme || '(blank)'}" is declared but not deterministically reopenable by this Core`,
    );
    return;
  }
...
function checkRow(
  ...
  if (values.recordState === 'indeterminate') {
    ...
    if (!familyKnown) fail(`${label} family "${values.family || '(blank)'}" is unsupported`);
    if (!typeKnown) fail(`${label} type "${values.type || '(blank)'}" is unsupported`);
  }
  if (
    familyKnown
    && values.type !== 'none'
    && !RELATION_FAMILY_TYPES[values.family as keyof typeof RELATION_FAMILY_TYPES]
      .includes(values.type as never)
  ) {
  ...
  const sourceKind = values.sourceKind as 'CC' | 'PKT';
  ...
  if (values.recordState === 'asserted') {
    ...
    if (values.targetKind === 'CC' || values.targetKind === 'PKT') {
      ...
      const source = model.corpus.sources.find(
        (candidate) => candidate.values.sourceId === values.targetSourceId,
      );
      if (!source) {
    ...
    if (
      values.recordState === 'unresolved-target'
    ...
    if (
      values.recordState === 'explicitly-absent'
    ...
    if (
      values.recordState === 'indeterminate'
...
export function runK2Relations(results: ResultCollector, model: RunModel): void {
  results.run('K2.16', 'typed relation structure and current-endpoint closure', (fail) => {
    ...
    if (!usesTypedRelations(version)) {
      if (structurePresent) {
        fail(
          `run format ${version || '(pre-versioned)'} must not be reinterpreted as ${RELATION_FORMAT}`,
        );
      }
    ...
    const markerCount = relations.document.lines.filter(
      (line) => /^\s*-\s*relation[_ -]format\s*:/i.test(line),
    ).length;
    ...
    if (
      relations.relationTables.length !== 1
      || relations.canonicalTables.length !== 1
    ) {
      fail(
        `run format ${version} requires exactly one canonical 17-column relation table; `
    ...
    for (const table of relations.relationTables) {
      const forbidden = table.normalizedHeader.filter((header) => (
        /(?:^| )(?:support|evidence role|load bearing|corroborative|contradictory)(?: |$)/.test(header)
      ));
      if (forbidden.length > 0) {
        fail(`relation table contains forbidden support/evidence-role field(s): ${forbidden.join(', ')}`);
      }
      if (
        table.normalizedHeader.length !== RELATION_TABLE_HEADER.length
        || table.normalizedHeader.some(
          (header, index) => header !== RELATION_TABLE_HEADER[index].replaceAll('_', ' '),
        )
      ) {
    ...
    const assertedTuples = new Map<string, string>();
    const subtypeScopes = new Map<string, RelationRow[]>();
    const familyScopes = new Map<string, RelationRow[]>();
    const sourceScopes = new Map<string, RelationRow[]>();
    for (const row of relations.rows) {
      const values = row.values;
      ...
      if (!sourceScopes.has(sourceScope)) sourceScopes.set(sourceScope, []);
      if (!familyScopes.has(familyScope)) familyScopes.set(familyScope, []);
      if (!subtypeScopes.has(subtypeScope)) subtypeScopes.set(subtypeScope, []);
    ...
    for (const rows of subtypeScopes.values()) {
      ...
      if (typedNulls.length > 1) {
        fail(
          `${typedNulls[0].values.sourceId} ${typedNulls[0].values.family}/${typedNulls[0].values.type} `
            + 'has more than one typed-null row',
        );
      }
      const states = new Set(rows.map((row) => row.values.recordState));
      if (
        states.has('explicitly-absent')
        && (states.has('asserted') || states.has('unresolved-target'))
      ) {
        fail(
          `${rows[0].values.sourceId} ${rows[0].values.family}/${rows[0].values.type} `
    ...
    for (const rows of familyScopes.values()) {
      const familyIndeterminate = rows.some((row) => (
        row.values.recordState === 'indeterminate' && row.values.type === 'none'
      ));
      const concrete = rows.some((row) => row.values.type !== 'none');
      if (familyIndeterminate && concrete) {
        fail(
          `${rows[0].values.sourceId} family ${rows[0].values.family} cannot combine `
            + 'family-level indeterminate with a concrete subtype row',
        );
      }
    }
    for (const rows of sourceScopes.values()) {
      const taxonomyIndeterminate = rows.some((row) => (
        row.values.recordState === 'indeterminate'
        && row.values.family === 'none'
        && row.values.type === 'none'
      ));
      if (taxonomyIndeterminate && rows.length > 1) {
        fail(
          `${rows[0].values.sourceId} taxonomy-level indeterminate cannot coexist with another relation row`,
        );
      }
    }

    const assertedUnitEdges = relations.rows.filter((row) => (
      row.values.recordState === 'asserted'
      && (row.values.targetKind === 'CC' || row.values.targetKind === 'PKT')
    ));
    const prohibitedSubgraphs: Array<{
      name: string;
      includes: (row: RelationRow) => boolean;
    }> = [
      {
    ...
    for (const subgraph of prohibitedSubgraphs) {
      const edges = assertedUnitEdges
        ...
        .map((row) => [row.values.sourceId, row.values.targetId] as const);
