...
const RUN_FORMAT_CAPABILITY_ADDITIONS: ReadonlyArray<{
  version: typeof SUPPORTED_RUN_FORMAT_VERSIONS[number];
  additions: readonly RunCapability[];
}> = [
  {
...
export function hasRunCapability(
  runFormatVersion: string,
  capability: RunCapability,
): boolean {
  const formatIndex = RUN_FORMAT_CAPABILITY_ADDITIONS.findIndex(
    (entry) => entry.version === runFormatVersion,
  );
...
export function usesFormalLayoutBindings(runFormatVersion: string): boolean {
...
export function loadRun(runDir: string): RunModel {
  ...
  const files = filePaths.filter((path) => (
    !relative(runDir, path).split(sep).join('/').startsWith('corpus/representation-assets/')
      && !opaqueLoci.has(relative(runDir, path).split(sep).join('/'))
  )).map((path) => ({
    path,
